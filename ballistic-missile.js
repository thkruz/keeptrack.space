...
var missileArray = [];
var MassRaidArray = [];
var lastMissileError = '';
var MAX_CHINESE_MISSILES = 252;
var MAX_USA_MISSILES = 350;
var MAX_RUSSIAN_MISSILES = 400;
var MAX_NORTH_KOREAN_MISSILES = 30;
...
var RussianICBM = [
  ...
  // 58.677, -22.765, 'North Atlantic (Delta IV)'
];
...
function MassRaid (time, BurnRate, RaidType) {

  var b = 500 - missilesInUse;

  if (RaidType === 'Russia') {
    var resetMissilesLaunched = false;
    var missilesLaunched = 0;
    for (var a = 0; a < RussianICBM.length / 4; a++) {
      if (resetMissilesLaunched) {
        missilesLaunched = 0;
        resetMissilesLaunched = false;
      }
      for (var i = 0; i < USATargets.length / 2; i++) {
        if (b <= 500 - MAX_RUSSIAN_MISSILES) continue; // Don't Launch more than 252 Missiles
        if (missilesLaunched > 30) continue; // 30 missiles per site
        if (Math.random() < 0.3) { // Skip over 70% of the cities to make it more random
          var LaunchTime = time + Math.random() * 240 * 1000;
          var success = Missile(RussianICBM[a * 4], RussianICBM[a * 4 + 1], USATargets[i * 2], USATargets[i * 2 + 1], 3, satSet.getSatData().length - b, LaunchTime, RussianICBM[a * 4 + 2], 30, 2.9, BurnRate, RussianICBM[a * 4 + 3]);
          missilesLaunched += success; // Add 1 if missile passed range checks
          b -= success;
        }
      }
      if (b <= 500 - MAX_RUSSIAN_MISSILES) continue;
      if (missilesLaunched <= 12 && i >= USATargets.length / 2) { // If less than 12 missiles launched redo that brigade
        a--;
        resetMissilesLaunched = false;
      } else {
        resetMissilesLaunched = true;
      }
    }
  } else if (RaidType === 'China') {
    resetMissilesLaunched = false;
    missilesLaunched = 0;
    for (a = 0; a < ChinaICBM.length / 4; a++) {
      if (resetMissilesLaunched) {
        missilesLaunched = 0;
        resetMissilesLaunched = false;
      }
      for (i = 0; i < USATargets.length / 2; i++) {
        if (b <= 500 - MAX_CHINESE_MISSILES) continue; // Don't Launch more than 252 Missiles
        if (missilesLaunched > 12) continue; // 12 missiles per brigade
        if (Math.random() < 0.3) { // Skip over 70% of the cities to make it more random
          LaunchTime = time + Math.random() * 240 * 1000;
          success = Missile(ChinaICBM[a * 4], ChinaICBM[a * 4 + 1], USATargets[i * 2], USATargets[i * 2 + 1], 3, satSet.getSatData().length - b, LaunchTime, ChinaICBM[a * 4 + 2], 30, 2.9, BurnRate, ChinaICBM[a * 4 + 3]);
          missilesLaunched += success; // Add 1 if missile passed range checks
          b -= success;
        }
      }
      if (b <= 500 - MAX_CHINESE_MISSILES) continue;
    ...
    resetMissilesLaunched = false;
    missilesLaunched = 0;
    for (a = 0; a < NorthKoreanBM.length / 4; a++) {
      if (resetMissilesLaunched) {
        missilesLaunched = 0;
        resetMissilesLaunched = false;
      }
      for (i = 0; i < USATargets.length / 2; i++) {
        if (b <= 500 - MAX_NORTH_KOREAN_MISSILES) continue; // Don't Launch more than 252 Missiles
        if (missilesLaunched > 12) continue; // 12 missiles per brigade
        if (Math.random() < 0.3) { // Skip over 70% of the cities to make it more random
          LaunchTime = time + Math.random() * 240 * 1000;
          success = Missile(NorthKoreanBM[a * 4], NorthKoreanBM[a * 4 + 1], USATargets[i * 2], USATargets[i * 2 + 1], 3, satSet.getSatData().length - b, LaunchTime, NorthKoreanBM[a * 4 + 2], 30, 2.9, BurnRate, NorthKoreanBM[a * 4 + 3]);
          missilesLaunched += success; // Add 1 if missile passed range checks
          b -= success;
        }
      }
      if (b <= 500 - MAX_NORTH_KOREAN_MISSILES) continue;
    ...
    var resetMissilesLaunched = false;
    var missilesLaunched = 0;
    for (var a = 0; a < UsaICBM.length / 4; a++) {
      var i = 0;
      console.log(a);
      if (resetMissilesLaunched) {
        missilesLaunched = 0;
      ...
      if (RaidType === 'USA2Russia') {
        for (i = 0; i < RussianICBM.length / 4; i++) {
          if (b <= 500 - MAX_USA_MISSILES) continue; // Don't Launch more than 350 Missiles
          if (missilesLaunched > 50) continue; // 50 missiles per site
          if (Math.random() < 0.3) { // Skip over 70% of the cities to make it more random
            var LaunchTime = time + Math.random() * 240 * 1000;
            var success = Missile(UsaICBM[a * 4], UsaICBM[a * 4 + 1], RussianICBM[i * 4], RussianICBM[i * 4 + 1], 3, satSet.getSatData().length - b, LaunchTime, UsaICBM[a * 4 + 2], 30, 2.9, BurnRate, UsaICBM[a * 4 + 3]);
            missilesLaunched += success; // Add 1 if missile passed range checks
            b -= success;
          }
        }
        if (b > 500 - MAX_USA_MISSILES) {
        ...
        for (i = 0; i < ChinaICBM.length / 4; i++) {
          if (b <= 500 - MAX_USA_MISSILES) continue; // Don't Launch more than 350 Missiles
          if (missilesLaunched > 50) continue; // 50 missiles per site
          if (Math.random() < 0.3) { // Skip over 70% of the cities to make it more random
            var LaunchTime = time + Math.random() * 240 * 1000;
            var success = Missile(UsaICBM[a * 4], UsaICBM[a * 4 + 1], ChinaICBM[i * 4], ChinaICBM[i * 4 + 1], 3, satSet.getSatData().length - b, LaunchTime, UsaICBM[a * 4 + 2], 30, 2.9, BurnRate, UsaICBM[a * 4 + 3]);
            missilesLaunched += success; // Add 1 if missile passed range checks
            b -= success;
          }
        }
        ...
        for (i = 0; i < NorthKoreanBM.length / 4; i++) {
          if (b <= 500 - 18) continue; // Don't Launch more than 15 Missiles
          if (missilesLaunched > 5) continue; // 30 missiles per site
          if (Math.random() < 0.3) { // Skip over 70% of the cities to make it more random
            var LaunchTime = time + Math.random() * 240 * 1000;
            var success = Missile(UsaICBM[a * 4], UsaICBM[a * 4 + 1], NorthKoreanBM[i * 4], NorthKoreanBM[i * 4 + 1], 3, satSet.getSatData().length - b, LaunchTime, UsaICBM[a * 4 + 2], 30, 2.9, BurnRate, UsaICBM[a * 4 + 3]);
            missilesLaunched += success; // Add 1 if missile passed range checks
            b -= success;
          }
        }
...
function ThrustFun (MassOut, Altitude, FuelArea, NozzleAltitude) {
  ...
  var Pa = Pressure(NozzleAltitude);                                                                // Ambient pressure used to calculate nozzle (Pa)
  var Pe = Pressure(Altitude);                                                                      // Actual Atmospheric Pressure (Pa)
...
function CoordinateCalculator (CurrentLatitude, CurrentLongitude, TargetLatitude, TargetLongitude) {
  ...
  for (var i = 0; i <= 2400; i++) {
    ...
    if (i === 2000) var GoalDistance = ((Sigma - DeltaSigma01) * r);
...
function RocketOrbit (OppositeList, AdjacentList) {
    ...
    // the rocket reaches. The iMathuts for this function are:
    // OppositeList: x components of the rockets path
    // AdjacentList: y components of the rockets path

    // TODO: CONFIRM THIS IS UNEEDED
}
function IterationFun (FuelArea, FuelMass, RocketArea, Altitude, RocketCasingMass, NozzleAltitude, drdt, dthetadt, Distance, ArcDistance, MassIn, AngleCoefficient) {
  ...
  if (Altitude < 1200000) var ThrustAngle = (90 - (AngleCoefficient) * (1.5336118956 + 0.00443173537387 * Altitude - (9.30373890848 * Math.pow(10, -8)) * Math.pow(Altitude, 2) +
  ...
  var Tatm = Temperature(Altitude);                                // (K)
  var Patm = Pressure(Altitude);                                   // (pa)
...
function Bisection (FuelArea1, FuelArea2, FuelMass, FuelVolume, RocketArea, Altitude, RocketCasingMass1, RocketCasingMass2, RocketCasingMass3, NozzleAltitude1, drdt, dthetadt, Distance, ArcDistance, MassIn, ArcLength, GoalDistance) {
  ...
  for (var i = 0; i < Steps; i++) {
    ...
    DistanceSteps.push(QuickRun(FuelArea1, FuelArea2, FuelMass, FuelVolume, RocketArea,
  ...
  ACNew = (AC1 + AC2) / 2;
  var qRunACNew = QuickRun(FuelArea1, FuelArea2, FuelMass, FuelVolume, RocketArea,
                                    Altitude, RocketCasingMass1, RocketCasingMass2, RocketCasingMass3,
  ...
  while (error > 0.01 && Math.abs(AC2 - AC1) >= 0.0001) {
    var ACNew = (AC1 + AC2) / 2;
    error = Math.abs((GoalDistance - QuickRun(
...
function QuickRun (FuelArea1, FuelArea2, FuelMass, FuelVolume, RocketArea, Altitude, RocketCasingMass1, RocketCasingMass2, RocketCasingMass3, NozzleAltitude1, drdt, dthetadt, Distance, ArcDistance, MassIn, AngleCoefficient) {
  ...
  while ((FuelMass / FuelDensity) / FuelVolume > 0.4 && Altitude >= 0) {
    iterationFunOutput = IterationFun(FuelArea1, FuelMass, RocketArea, Altitude, RocketCasingMass1, NozzleAltitude1, drdt, dthetadt, Distance, ArcDistance, MassIn, AngleCoefficient);
  ...
  while ((FuelMass / FuelDensity) / FuelVolume > 0.19 && Altitude >= 0) {
    iterationFunOutput = IterationFun(FuelArea1, FuelMass, RocketArea, Altitude, RocketCasingMass2, NozzleAltitude2, drdt, dthetadt, Distance, ArcDistance, MassIn, AngleCoefficient);
    FuelMass = iterationFunOutput[0];
    RocketMass = iterationFunOutput[1];
    Tatm = iterationFunOutput[2];
    Patm = iterationFunOutput[3];
  ...
  while ((FuelMass / FuelDensity) / FuelVolume > 0 && Altitude >= 0) {
    iterationFunOutput = IterationFun(FuelArea2, FuelMass, RocketArea, Altitude, RocketCasingMass3, NozzleAltitude3, drdt, dthetadt, Distance, ArcDistance, MassIn, AngleCoefficient);
    FuelMass = iterationFunOutput[0];
    RocketMass = iterationFunOutput[1];
    Tatm = iterationFunOutput[2];
    Patm = iterationFunOutput[3];
...
function Missile (CurrentLatitude, CurrentLongitude, TargetLatitude, TargetLongitude, NumberWarheads, MissileObjectNum, CurrentTime, MissileDesc, Length, Diameter, NewBurnRate, MaxMissileRange) {
  ...
  if (TargetLatitude > 90 || TargetLatitude < -90) {
    ...
    lastMissileError = 'Error: Target Latitude must be<br>between 90 and -90 degrees';
    return 0;
  }
  if (TargetLongitude > 180 || TargetLongitude < -180) {
    // console.error('Error: Target Longitude must be between 180 and -180 degrees');
    lastMissileError = 'Error: Target Longitude must be<br>between 90 and -90 degrees';
    return 0;
  }
  ...
  calculatedCoordinates = CoordinateCalculator(CurrentLatitude, CurrentLongitude, TargetLatitude, TargetLongitude);
  ...
  if (ArcLength < 320000) {
    ...
    lastMissileError = 'Error: This missile has a minimum distance of 320 km.';
    return 0;
  }
  ...
  if (ArcLength > MaxMissileRange * 1000) {
    ...
    // console.error('Please choose different target coordinates.');
    lastMissileError = 'Error: This missile has a maximum distance of ' + MaxMissileRange + ' km.';
    return 0;
  }
  ...
  var AngleCoefficient = Bisection(FuelArea1, FuelArea2, FuelMass, FuelVolume, RocketArea, Altitude, RocketCasingMass1, RocketCasingMass2, RocketCasingMass3, NozzleAltitude1, drdt, dthetadt, Distance, ArcDistance, MassIn, ArcLength, GoalDistance);

  while ((FuelMass / FuelDensity) / FuelVolume > 0.4 && Altitude >= 0) {
    var iterationFunOutput = IterationFun(FuelArea1, FuelMass, RocketArea, Altitude, RocketCasingMass1, NozzleAltitude1, drdt, dthetadt, Distance, ArcDistance, MassIn, AngleCoefficient);
    FuelMass = iterationFunOutput[0];
    RocketMass = iterationFunOutput[1];
    Tatm = iterationFunOutput[2];
    Patm = iterationFunOutput[3];

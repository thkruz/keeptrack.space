function CanvasRecorder(e, o) {
  ;(this.start = function () {
    new Promise(function (e, o) {
      ...
      e(
        'https:' === window.location.protocol
          ? 'getDisplayMedia' in navigator
            ? navigator
                .getDisplayMedia(n)
                .catch((e) => (console.warn('Error:' + e), null))
      ...
      for (let e in s)
        if (MediaRecorder.isTypeSupported(s[e])) {
          t = s[e]
          break
        }
      null == t && console.log('No supported type found for MediaRecorder')
      let l = { mimeType: t, videoBitsPerSecond: o || 1e7 }
      n = []
      try {
        d = new MediaRecorder(a, l)
      } catch (e) {
        return (
          (isVideoRecording = !1),
          $('#menu-record').removeClass('bmenu-item-selected'),
          void console.warn('Exception while creating MediaRecorder:', e)
        )
      }
      console.log('Created MediaRecorder', d, 'with options', l),
        (d.onstop = c),
        (d.ondataavailable = i),
        d.start(100),
        console.log('MediaRecorder started', d)
    })
  }),
    (this.stop = function () {
      d.stop(), console.log('Recorded Blobs: ', n), (r.controls = !0)
    }),
    (this.save = function (e) {
      const o = e || 'recording.webm',
        d = new Blob(n, { type: t }),
        r = window.URL.createObjectURL(d),

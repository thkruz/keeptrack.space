var astroCalcLib = new AstroCalcLib();
function AstroCalcLib () {
  this.version = '1.0.2';

  /*
   * astroCalcLib v1.0.3
   * (c) 2018 Theodore Kruczek
   * https://github.com/thkruz/astroCalcLib
   * License: MIT
   */
   ...
   */
   ...
   j2 = 0.00108262998905,
   j3 = -0.00000253215306;
  AstroCalcLib.constants = {};
  AstroCalcLib.constants.pi = pi;
  AstroCalcLib.constants.twoPi = pi * 2;
  AstroCalcLib.constants.deg2rad = pi / 180.0;
  AstroCalcLib.constants.rad2deg = 180 / pi;
  AstroCalcLib.constants.minutesPerDay = 1440.0;
  AstroCalcLib.constants.mu = mu;
  AstroCalcLib.constants.earthRadius = earthRadius;
  AstroCalcLib.constants.xke = xke;
  AstroCalcLib.constants.tumin = 1.0 / xke;
  AstroCalcLib.constants.j2 = j2;
  AstroCalcLib.constants.j3 = j3;
  AstroCalcLib.constants.j4 = -0.00000161098761;
  AstroCalcLib.constants.j3oj2 = j3 / j2;
  AstroCalcLib.constants.x2o3 = 2.0 / 3.0;
  AstroCalcLib.constants.vkmpersec = earthRadius * xke / 60.0;
  dspace = function (dspaceParameters) {
    /*-----------------------------------------------------------------------------
    *
    *                           procedure dspace
    *
    ...
    *    effects have been averaged over no revolutions of the this.
    *    (mean motion)
    *
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    var constants = AstroCalcLib.constants;
  ...
  dpper = function (satrec, dpperParameters) {
    'use strict';
    /* -----------------------------------------------------------------------------
    *
    *                           procedure dpper
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    if (init === 'n'){
      ...
      if (inclp >= 0.2) {
        ...
        alfdp       = alfdp + dalf;
        betdp       = betdp + dbet;
        nodep       = nodep % AstroCalcLib.constants.twoPi;
        ...
        if (Math.abs(xnoh - nodep) > AstroCalcLib.constants.pi) {
          if (nodep < xnoh){
            nodep = nodep + AstroCalcLib.constants.twoPi;
          }
          else{
  ...
  AstroCalcLib.prototype.degreesLat = function (){
      'use strict';
      var constants = AstroCalcLib.constants;
      return function(radians) {
          if (radians > AstroCalcLib.constants.pi/2 || radians < (-AstroCalcLib.constants.pi/2)){
              return 'Err';
          }
          var degrees = (radians/AstroCalcLib.constants.pi*180);
          if (degrees < 0){
              degrees = degrees;
          }
          else{
  ...
  AstroCalcLib.prototype.degreesLong = function () {
      'use strict';
      var constants = AstroCalcLib.constants;
      return function (radians) {
          var degrees = (radians/AstroCalcLib.constants.pi*180) % (360);
          if (degrees > 180){
              degrees = 360 - degrees;
          }
          else if (degrees < -180){
  ...
  AstroCalcLib.prototype.ecfToEci = function (ecfCoords, gmst) {
      'use strict';
  ...
  AstroCalcLib.prototype.geodeticToEcf = function(geodeticCoords) {
    'use strict';
  ...
  AstroCalcLib.prototype.topocentric = function (observerCoords, satelliteCoords) {
    'use strict';
      ...
      var observerEcf = this.geodeticToEcf(observerCoords);
  ...
  AstroCalcLib.prototype.topocentricToLookAngles = function (topocentric) {
    ...
    var Az      = Math.atan2 (-topE, topS) + AstroCalcLib.constants.pi;
  ...
  AstroCalcLib.prototype.ecfToLookAngles = function (observerCoordsEcf, satelliteCoordsEcf) {
    'use strict';
    var topocentricCoords = topocentric(observerCoordsEcf, satelliteCoordsEcf);
    return topocentricToLookAngles(topocentricCoords);
  };
  AstroCalcLib.prototype.eciToEcf = function (eciCoords, gmst) {
    'use strict';
      // ccar.colorado.edu/ASEN5070/handouts/coordsys.doc
      //
      // [X]     [C -S  0][X]
  ...
  AstroCalcLib.prototype.eciToGeodetic = function (eciCoords, gmst) {
    'use strict';
  ...
  AstroCalcLib.prototype.dopplerFactor = function (myLocation, position, velocity) {
    'use strict';
  ...
  AstroCalcLib.prototype.sgp4 = function (satrec, tsince) {
    // 'use strict';
    /*-----------------------------------------------------------------------------
     *
     *                             procedure sgp4
    ...
    var vkmpersec = AstroCalcLib.constants.vkmpersec;
    ...
    if (pl < 0.0) {
        ...
        v = {x: 0.0, y: 0.0, z: 0.0};
        v.x = (mvt * ux + rvdot * vx) * AstroCalcLib.constants.vkmpersec;
        v.y = (mvt * uy + rvdot * vy) * AstroCalcLib.constants.vkmpersec;
        v.z = (mvt * uz + rvdot * vz) * AstroCalcLib.constants.vkmpersec;
    }
  ...
  AstroCalcLib.prototype.propagate = function () {
    'use strict';
    //Return a position and velocity vector for a given date and time.
    var satrec = arguments[0],
        date = Array.prototype.slice.call(arguments, 1),
        j = jday.apply(null, date),
        m = (j - satrec.jdsatepoch) * AstroCalcLib.constants.minutesPerDay;
    return sgp4(satrec, m);
  };
  AstroCalcLib.prototype.dscom = function (dscomParameters) {
    'use strict';
    /*-----------------------------------------------------------------------------
     *
     *                           procedure dscom
    ...
    xnodce = (4.5236020 - 9.2422029e-4 * day) % AstroCalcLib.constants.twoPi;
    ...
    zmol = (4.7199672 + 0.22997150  * day - gam)   % AstroCalcLib.constants.twoPi;
    zmos = (6.2565837 + 0.017201977 * day)         % AstroCalcLib.constants.twoPi;
  ...
  AstroCalcLib.prototype.dsinit = function (dsinitParameters) {
    'use strict';
    /*-----------------------------------------------------------------------------
     *
     *                           procedure dsinit
    ...
    if (inclm < 5.2359877e-2 || inclm > AstroCalcLib.constants.pi - 5.2359877e-2){
        shs = 0.0;
    }
    if (sinim !== 0.0){
        shs = shs / sinim;
    ...
    //  ----------- calculate deep space resonance effects --------
    var dndt    = 0.0;
    theta   = (gsto + tc * rptim) % AstroCalcLib.constants.twoPi;
    em          = em + dedt * t;
    inclm       = inclm + didt * t;
    argpm       = argpm + domdt * t;
    nodem       = nodem + dnodt * t;
    ...
    if (irez !== 0) {
        aonv = Math.pow(nm / AstroCalcLib.constants.xke, x2o3);
        ...
        if (irez === 2) {
            ...
            xlamo = (mo     + nodeo + nodeo -  theta    - theta) % AstroCalcLib.constants.twoPi;
        ...
        if (irez === 1) {
            ...
            xlamo = (mo + nodeo + argpo - theta) % AstroCalcLib.constants.twoPi;
  ...
  AstroCalcLib.prototype.initl = function (initlParameters) {
    'use strict';
    /*-----------------------------------------------------------------------------
     *
     *                           procedure initl
    ...
    ak = Math.pow(AstroCalcLib.constants.xke / no, AstroCalcLib.constants.x2o3);
    d1 = 0.75 * AstroCalcLib.constants.j2 * (3.0 * cosio2 - 1.0) / (rteosq * omeosq);
    ...
    var ao = Math.pow(AstroCalcLib.constants.xke / no, AstroCalcLib.constants.x2o3);
    ...
    if (opsmode === 'a') {
        ...
        var c1p2p = c1 + AstroCalcLib.constants.twoPi;
        gsto = ( thgr70 + c1 * ds70 + c1p2p * tfrac + ts70 * ts70 * fk5r) % AstroCalcLib.constants.twoPi;
        if (gsto < 0.0) {
            gsto = gsto + AstroCalcLib.constants.twoPi;
        }
    }
    else {
        gsto = this.gstime(epoch + 2433281.5);
    }
  ...
  AstroCalcLib.prototype.sgp4init = function (satrec, sgp4initParameters) {
    'use strict';
    /*-----------------------------------------------------------------------------
     *
     *                             procedure sgp4init
    ...
    var ss     = 78.0 / AstroCalcLib.constants.earthRadius + 1.0;
    //  sgp4fix use multiply for speed instead of pow
    var qzms2ttemp = (120.0 - 78.0) / AstroCalcLib.constants.earthRadius;
    ...
    var initlResult= this.initl(initlParameters);
    ...
    if (omeosq >= 0.0 || satrec.no >= 0.0){
        satrec.isimp = 0;
        if (rp < 220.0 / AstroCalcLib.constants.earthRadius + 1.0){
            satrec.isimp = 1;
        }
        sfour  = ss;
        qzms24 = qzms2t;
        perige = (rp - 1.0) * AstroCalcLib.constants.earthRadius;
        ...
        if (perige < 156.0){
            ...
            //  sgp4fix use multiply for speed instead of pow
            var qzms24temp =  (120.0 - sfour) / AstroCalcLib.constants.earthRadius;
            qzms24 = qzms24temp * qzms24temp * qzms24temp * qzms24temp;
            sfour  = sfour / AstroCalcLib.constants.earthRadius + 1.0;
        }
        pinvsq = 1.0 / posq;
        ...
        cc2   = coef1 * satrec.no * (ao * (1.0 + 1.5 * etasq + eeta *
                (4.0 + etasq)) + 0.375 * AstroCalcLib.constants.j2 * tsi / psisq * satrec.con41 *
                (8.0 + 3.0 * etasq * (8.0 + etasq)));
        satrec.cc1   = satrec.bstar * cc2;
        cc3   = 0.0;
        if (satrec.ecco > 1.0e-4) {
            cc3 = -2.0 * coef * tsi * AstroCalcLib.constants.j3oj2 * satrec.no * sinio / satrec.ecco;
        }
        satrec.x1mth2 = 1.0 - cosio2;
        satrec.cc4    = 2.0 * satrec.no * coef1 * ao * omeosq *
                        (satrec.eta * (2.0 + 0.5 * etasq) + satrec.ecco *
                        (0.5 + 2.0 * etasq) - AstroCalcLib.constants.j2 * tsi / (ao * psisq) *
        ...
        temp1 = 1.5 * AstroCalcLib.constants.j2 * pinvsq * satrec.no;
        temp2 = 0.5 * temp1 * AstroCalcLib.constants.j2 * pinvsq;
        temp3 = -0.46875 * AstroCalcLib.constants.j4 * pinvsq * pinvsq * satrec.no;
        ...
        if (Math.abs(cosio+1.0) > 1.5e-12){
            satrec.xlcof = -0.25 * AstroCalcLib.constants.j3oj2 * sinio * (3.0 + 5.0 * cosio) / (1.0 + cosio);
        }
        else{
            satrec.xlcof = -0.25 * AstroCalcLib.constants.j3oj2 * sinio * (3.0 + 5.0 * cosio) / temp4;
        }
        satrec.aycof   = -0.5 * AstroCalcLib.constants.j3oj2 * sinio;
        //  sgp4fix use multiply for speed instead of pow
        ...
        //  --------------- deep space initialization -------------
        if (2*AstroCalcLib.constants.pi / satrec.no >= 225.0){
            satrec.method = 'd';
            satrec.isimp  = 1;
            tc    =  0.0;
            inclm = satrec.inclo;
            ...
            var dscomResult = this.dscom(dscomParameters);
            ...
            var dsinitResult = this.dsinit( dsinitParameters );
  ...
  AstroCalcLib.prototype.twoline2satrec = function (longstr1, longstr2) {
    'use strict';
    ...
    var opsmode = 'i';
    var xpdotp   =  1440.0 / (2.0 *AstroCalcLib.constants.pi);  //  229.1831180523293;
    var revnum = 0;
    var elnum = 0;
    var year = 0;
    ...
    satrec.a    = Math.pow( satrec.no*AstroCalcLib.constants.tumin , (-2.0/3.0) );
    satrec.ndot = satrec.ndot  / (xpdotp*1440.0);  //   ? * minperday
    satrec.nddot= satrec.nddot / (xpdotp*1440.0*1440);

    //  ---- find standard orbital elements ----
    satrec.inclo = satrec.inclo  * AstroCalcLib.constants.deg2rad;
    satrec.nodeo = satrec.nodeo  * AstroCalcLib.constants.deg2rad;
    satrec.argpo = satrec.argpo  * AstroCalcLib.constants.deg2rad;
    satrec.mo    = satrec.mo     * AstroCalcLib.constants.deg2rad;
    ...
    var mdhmsResult = this.days2mdhms(year, satrec.epochdays);
    ...
    this.sgp4init(satrec, sgp4initParameters );

    return satrec;
  };

  AstroCalcLib.prototype.days2mdhms = function (year, days) {
    'use strict';
    /* -----------------------------------------------------------------------------
    *
  ...
  AstroCalcLib.prototype.gstime = function (jdut1) {
    'use strict';
    /* -----------------------------------------------------------------------------
    *
    *                           function gstime
    ...
    * --------------------------------------------------------------------------- */
    ...
    temp = (temp * AstroCalcLib.constants.deg2rad / 240.0) % AstroCalcLib.constants.twoPi; // 360/86400 = 1/240, to deg, to rad

    //  ------------------------ check quadrants ---------------------
    if (temp < 0.0){
      temp += AstroCalcLib.constants.twoPi;
    }
    return temp;
  };

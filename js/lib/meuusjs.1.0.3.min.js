...
A.Coord = {
  ...
  eqToEcl: function (a, b) {
    ...
    return new A.EclCoord(
      Math.atan2(c * g + (d / e) * f, Math.cos(a.ra)),
      Math.asin(d * g - e * f * c),
    )
  },
  eclToEq: function (a, b) {
    ...
    0 > k && (k += 2 * Math.PI)
    return new A.EqCoord(k, Math.asin(d * g + e * f * c))
  },
  eqToHz: function (a, b, c) {
    ...
    return new A.HzCoord(
      Math.atan2(Math.sin(c), d * e - (f / a) * b),
      Math.asin(e * f + b * a * d),
    )
  },
}
A.DeltaT = {
  ...
  estimate: function (a) {
    ...
    return -500 > b
      ...
      : 1941 > b
      ? ((b -= 1920),
        21.2 + 0.84493 * b - 0.0761 * a(b, 2) + 0.0020936 * a(b, 3))
      : 1961 > b
...
A.JulianDay.calendarGregorianToJD = function (a, b, c) {
  ...
  return (
    Math.floor((36525 * (a + 4716)) / 100) +
    Math.floor((306 * (b + 1)) / 10) +
    (2 - d + Math.floor(d / 4)) +
    c -
    1524.5
  )
...
A.JulianDay.jdToCalendar = function (a) {
  ...
  var b = a[0],
    c = b
  2299151 <= b &&
    ((c = Math.floor((100 * b - 186721625) / 3652425)),
...
A.Moon = {
  ...
  approxTransit: function (a, b) {
    ...
    return A.Rise.approxTransit(
      b,
      A.Sidereal.apparent0UT(c),
      A.Moon.apparentTopocentric(c, b).eq,
    )
  },
  ...
  parallacticAngle: function (a, b, c) {
    return Math.atan2(
      Math.sin(b),
      Math.tan(a) * Math.cos(c) - Math.sin(c) * Math.cos(b),
    )
  },
  geocentricPosition: function (a) {
    ...
    for (h = 0; h < A.Moon.tb.length; h++)
      switch (
        ((l = A.Moon.tb[h]),
        (q = Math.sin(d * l[0] + e * l[1] + f * l[2] + g * l[3])),
        l[1])
      ) {
        case 0:
          k += l[4] * q
...
A.MoonIllum = {
  ...
  positionAngle: function (a, b) {
    ...
    return Math.atan2(
      c * Math.sin(b.ra - a.ra),
      Math.sin(b.dec) * Math.cos(a.dec) -
        c * Math.sin(a.dec) * Math.cos(b.ra - a.ra),
    )
  },
  _coselong: function (a, b) {
    return (
      Math.sin(b.dec) * Math.sin(a.dec) +
      Math.cos(b.dec) * Math.cos(a.dec) * Math.cos(b.ra - a.ra)
    )
  },
}
A.Nutation = {
  nutation: function (a) {
    ...
    for (
      var b =
          (A.Math.horner(a, [297.85036, 445267.11148, -0.0019142, 1 / 189474]) *
            Math.PI) /
          180,
        c =
          (A.Math.horner(a, [357.52772, 35999.05034, -1.603e-4, -1 / 3e5]) *
            Math.PI) /
          180,
        d =
          (A.Math.horner(a, [134.96298, 477198.867398, 0.0086972, 1 / 5620]) *
            Math.PI) /
          180,
        e =
          (A.Math.horner(a, [93.27191, 483202.017538, -0.0036825, 1 / 327270]) *
            Math.PI) /
          180,
        f =
          (A.Math.horner(a, [125.04452, -1934.136261, 0.0020708, 1 / 45e4]) *
            Math.PI) /
          180,
        g = 0,
        k = 0,
...
A.Parallax = {
  ...
  topocentric: function (a, b, c, d, e, f) {
    ...
    return new A.EqCoord(
      a.ra + e,
      Math.atan2((Math.sin(a.dec) - c * b) * Math.cos(e), g - d * b * f),
    )
  },
  topocentric2: function (a, b, c, d, e, f) {
    ...
    f = Math.cos(a.dec)
    return new A.EqCoord(
      a.ra + (-b * d * Math.sin(e)) / f,
      a.dec + -b * (c * f - d * Math.cos(e) * Math.sin(a.dec)),
    )
  },
}
A.Refraction = {
...
A.Rise = {
  ...
  times: function (a, b, c, d, e) {
    ...
    g.transit = A.Math.pMod(
      g.transit - (e - (43200 * (a.lng + h)) / Math.PI),
      86400,
    )
...
A.Sidereal = {
  ...
  apparent0UT: function (a) {
    ...
    b =
      A.Math.horner((b[0] - 0.5 - A.J2000) / 36525, A.Sidereal.iau82) +
      86636.55536784 * b[1]
...
A.Solar = {
  ...
  apparentEquatorial: function (a) {
    ...
    a =
      A.Nutation.meanObliquityLaskar(a) +
      ((0.00256 * Math.PI) / 180) * Math.cos(c)
    c = Math.sin(b)
    return new A.EqCoord(
      Math.atan2(Math.cos(a) * c, Math.cos(b)),
      Math.asin(Math.sin(a) * c),
    )
  },
  apparentTopocentric: function (a, b, c) {
    var d = A.Solar.apparentEquatorial(a),
      e = A.Globe.parallaxConstants(b.lat, b.h)
    c || (c = A.Sidereal.apparentInRa(a))
    return A.Parallax.topocentric2(
      d,
  ...
  approxTransit: function (a, b) {
    ...
    return A.Rise.approxTransit(
      b,
      A.Sidereal.apparent0UT(c),
      A.Solar.apparentTopocentric(c, b),
    )
  },
  approxTimes: function (a, b) {
    var c = a.startOfDay(),
    ...
    return A.Rise.approxTimes(b, e, c, d)
  },
  times: function (a, b) {
    var c = a.startOfDay(),
  ...
  meanAnomaly: function (a) {
    return (
      (A.Math.horner(a, [357.52911, 35999.05029, -1.537e-4]) * Math.PI) / 180
    )
  },
  trueLongitude: function (a) {
    var b =
        (A.Math.horner(a, [280.46646, 36000.76983, 3.032e-4]) * Math.PI) / 180,
      c = A.Solar.meanAnomaly(a)
    a =
      ((A.Math.horner(a, [1.914602, -0.004817, -1.4e-5]) * Math.sin(c) +
        (0.019993 - 1.01e-4 * a) * Math.sin(2 * c) +
        2.89e-4 * Math.sin(3 * c)) *
        Math.PI) /
      180
    return {
  ...
  apparentLongitude: function (a, b) {
    b || (b = A.Solar.node(a))
    return (
      A.Solar.trueLongitude(a).s -
      (0.00569 * Math.PI) / 180 -
      ((0.00478 * Math.PI) / 180) * Math.sin(b)
    )
  },
  node: function (a) {
...
A.Solistice = {
  march: function (a) {
    return 1e3 > a
      ? A.Solistice._eq(a, A.Solistice.mc0)
      : A.Solistice._eq(a - 2e3, A.Solistice.mc2)
  },
  june: function (a) {
    return 1e3 > a
      ? A.Solistice._eq(a, A.Solistice.jc0)
      : A.Solistice._eq(a - 2e3, A.Solistice.jc2)
  },
  september: function (a) {
    return 1e3 > a
      ? A.Solistice._eq(a, A.Solistice.sc0)
      : A.Solistice._eq(a - 2e3, A.Solistice.sc2)
  },
  december: function (a) {
    return 1e3 > a
      ? A.Solistice._eq(a, A.Solistice.dc0)
      : A.Solistice._eq(a - 2e3, A.Solistice.dc2)
  },
  _eq: function (a, b) {
    for (
      var c = A.Math.horner(0.001 * a, b),
        d = (c - A.J2000) / A.JulianCentury,
        e = ((35999.373 * Math.PI) / 180) * d - (2.47 * Math.PI) / 180,
        e = 1 + 0.0334 * Math.cos(e) + 7e-4 * Math.cos(2 * e),
        f = 0,
        g = this.terms.length - 1;
      0 <= g;
      g--
    )
      var k = this.terms[g],

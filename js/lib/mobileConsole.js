  ...
  var setLineStyle = (function () {
      ...
      return function (element, type, msg) {
        if (status.initialized) {
          ...
          //has dot?
          if (
            element.childNodes[0].childNodes[0].className.indexOf('dot') !== -1
          ) {
    ...
    getLink = function (href, textString) {
      ...
      HTMLurl.innerHTML =
        textString || href.split('\\').pop().split('/').filter(Boolean).pop()
      return HTMLurl
    },
    toggleHeight = function () {
      if (status.initialized) {
        ...
        setCSS(document.body, {
          paddingBottom:
            existingPadding +
            Math.abs(parseInt(newHeight, 10) + elements.topbar.offsetHeight) +
            'px',
        })
  ...
  function initConsoleElement() {
    ...
    function attach(console) {
      ...
      setCSS(document.body, {
        paddingBottom:
          existingPadding +
          Math.abs(console.offsetHeight + elements.topbar.offsetHeight) +
          'px',
      })
      elements.scrollcontainer.scrollTop = elements.scrollcontainer.scrollHeight
      ...
      try {
        ...
        if (isRepeat(message, method) && method.indexOf('time') === -1) {
          ...
          elements.table.lastChild.countDot =
            elements.table.lastChild.countDot || elements.dot.cloneNode(false)
          ...
          if (
            ...
            rightContainer.appendChild(
              setCSS(getLink(stackTrace[0].url, stackTrace[0].linkText), {
                color: '#808080',
              }),
            )
          }
    ...
    function toggleAutoComplete(show) {
      var hidden =
        (elements.autocomplete.currentStyle
          ...
          : window.getComputedStyle(elements.autocomplete, null).display) ===
        'none'
        ...
        if (tgt === elements.consoleinput) {
          if (e.key === 'Enter' || e.keyCode === 13) {
            ...
            if (status.acActive) {
              ...
              elements.consoleinput.value =
                typeof hist[history.input.commandIdx] === 'undefined'
                  ? ''

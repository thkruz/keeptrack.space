...
var mobileConsole = (function () {
        ...
        originalConsole = {
            log:
                console && typeof console.log === 'function'
                    ...
                    : missingMethod,
            info:
                console && typeof console.info === 'function'
                    ? console.info.bind(console)
                    : missingMethod,
            dir:
                console && typeof console.dir === 'function'
            ...
            group:
                console && typeof console.group === 'function'
                    ...
                    : missingMethod,
            groupEnd:
                console && typeof console.groupEnd === 'function'
            ...
            warn:
                console && typeof console.warn === 'function'
                    ...
                    : missingMethod,
            error:
                console && typeof console.error === 'function'
            ...
            trace:
                console && typeof console.trace === 'function'
                    ...
                    : missingMethod,
            clear:
                console && typeof console.clear === 'function'
        ...
        if (history) {
            ...
            if (window.sessionStorage) {
                sessionStorage.setItem(
                    'mobileConsoleCommandHistory',
                    JSON.stringify(history.input.commands)
                );
            }
        }
    }
    ...
    var setLineStyle = (function () {
            ...
            return function (element, type, msg) {
                if (status.initialized) {
                    color =
                        ...
                        msg === htmlToString(messages.empty)
                            ? { color: '#808080' }
                            ...
                            : lineStyles.log.text;
                    dot =
                        typeof lineStyles(type) !== 'undefined'
                            ? lineStyles(type).dot
                            : lineStyles.log.dot;
                    setCSS(element, color);
                    //has dot?
                    if (
                        element.childNodes[0].childNodes[0].className.indexOf(
                            'dot'
                        ) !== -1
                    ) {
                        setCSS(
        ...
        getLink = function (href, textString) {
            ...
            HTMLurl.innerHTML =
                textString ||
                href.split('\\').pop().split('/').filter(Boolean).pop();
            return HTMLurl;
        },
        toggleHeight = function () {
            if (status.initialized) {
                var existingPadding =
                    ...
                    Math.abs(
                        elements.base.offsetHeight +
                            elements.topbar.offsetHeight
                    );
                var newHeight = elements.base.minimized
                    ? getMaxHeight() + 'px'
                    : '0px';
                ...
                elements.buttons.toggler.innerHTML = elements.base.minimized
                    ? elements.arrowDown
                    : elements.arrowUp;
                elements.buttons.toggler.setAttribute(
                    'title',
                    elements.base.minimized
                        ? 'Minimize console'
                        : 'Maximize console'
                );
    ...
    function initConsoleElement() {
        ...
        function toggleScroll() {
            elements.scrollcontainer.scrollTop =
                elements.scrollcontainer.scrollHeight;
        ...
        function assemble() {
            ...
            elements.buttons.closer.setAttribute(
                'title',
            ...
            while (i--) {
                elements.buttons[
                    options.methods[i]
                ] = elements.button.cloneNode(false);
                elements.buttons[options.methods[i]].innerHTML =
                    options.methods[i].charAt(0).toUpperCase() +
                    options.methods[i].slice(1);
            ...
            for (key in elements.buttons) {
                if (elements.buttons.hasOwnProperty(key)) {
                    elements.topbar.insertBefore(
                        elements.buttons[key],
                        elements.topbar.firstChild
                    );
                }
            }
        ...
        function attach(console) {
            ...
            setCSS(document.body, {
                paddingBottom:
                    existingPadding +
                    Math.abs(
                        console.offsetHeight + elements.topbar.offsetHeight
                    ) +
                    'px',
            });
            elements.scrollcontainer.scrollTop =
        ...
        function toggleLogType(e) {
            ...
            var elems = elements.lines[logType],
                i = elems.length;
            button.toggled =
                typeof button.toggled === 'undefined' ? true : !button.toggled;
        ...
        function setBinds() {
            ...
            while (i--) {
                if (methods[i] !== 'clear') {
                    if (options.browserinfo.evtLstn) {
                        elements.buttons[methods[i]].addEventListener(
                            'click',
                            toggleLogType,
                            false
                        );
                    } else {
                        elements.buttons[methods[i]].attachEvent(
                            'onclick',
                            toggleLogType
                        );
                    }
            ...
            if (options.browserinfo.evtLstn) {
                elements.buttons.toggler.addEventListener(
                    'click',
                    toggleHeight,
                    false
                );
                elements.buttons.closer.addEventListener(
                    'click',
                    destroyConsole,
                    false
                );
                elements.buttons.clear.addEventListener(
                    'click',
                    console.clear,
                    false
                );
    ...
    function initConsole() {
        ...
        function urlFromString(string) {
            ...
            var match,
                uriPattern = /\b((?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?Â«Â»â€œâ€â€˜â€™]))/gi;
            try {
        ...
        function preFilterTrace(array) {
            ...
            if (
                newArray[0].indexOf('console.') !== -1 ||
                newArray[0].indexOf('console[method]') !== -1
            ) {
                newArray.shift();
            }
            if (newArray.length > 0) {
                isCommandLine =
                    newArray[newArray.length - 1].indexOf('keydown') !== -1;
        ...
        function formatStackTrace(trace, origtrace) {
            ...
            var stackTraceOrig =
                typeof trace !== 'undefined' && typeof trace[4] !== 'undefined'
                    ? trace[4].stack
                    : undefined;
            ...
            var traceToProcess =
                    origtrace && origtrace !== '' ? origtrace : stackTraceOrig,
                i,
                lines,
                url,
                txt,
        ...
        function traceToTable(table, trace) {
            ...
            while (i--) {
                ...
                tdRight.innerHTML = '&nbsp;@&nbsp;';
                tdRight.appendChild(
                    getLink(trace[i].url || '', trace[i].linkText)
                );
        ...
        function colorizeData(key, value) {
            ...
            if (typeof value === 'string') {
                ...
                if (value.length > options.truncate) {
                    value =
                        '"' +
                        String(value).substring(0, options.truncate) +
                        '" [...] <br/><span style="color:#FF0000;text-decoration: underline;">Note: string was truncated to ' +
                        options.truncate +
                        ' chars</span>';
                } else {
                    value = '"' + value + '"';
                ...
                if (isEmpty(value)) {
                    ...
                    var tempTable = createElem('table', 'stackTraceSubTable', {
                            ...
                            display: 'none',
                            marginLeft: '10px',
                            marginTop: options.browserinfo.isMobile
                                ? '8px'
                                : '4px',
                            tableLayout: 'auto',
                            maxWidth: '100%',
                    ...
                    wrap.appendChild(
                        objectToTable(tempTable, value).cloneNode(true)
                    );
                ...
                while (i--) {
                    hidden =
                        (tables[i].currentStyle
                            ...
                            : window.getComputedStyle(tables[i], null)
                                  .display) === 'none';
        ...
        function newConsole() {
            try {
                ...
                var method = arguments[0],
                    ...
                    isHTMLElement,
                    message =
                        typeof arguments[1].newMessage !== 'undefined'
                            ? arguments[1].newMessage
                            : undefined,
                    stackTrace =
                        typeof arguments[1].newStackTrace !== 'undefined'
                            ? arguments[1].newStackTrace
                            : undefined;
                ...
                if (
                    ...
                    if (
                        ...
                        message = isElement(message)
                            ? htmlToString(
                                  message.outerHTML.match(/<(.*?)>/g)[0] +
                                      '...' +
                                      message.outerHTML.match(/<(.*?)>/g).pop()
                              ) //gets e.g. <div>...</div>
                    ...
                    switch (method) {
                        case 'assert':
                            if (message[0] === false) {
                                msgContainer.innerHTML =
                                    arrows + 'Assertion failed: ' + message[1];
                            }
                            stackTable = traceToTable(
                                elements.stackTraceTable.cloneNode(false),
                                stackTrace
                            );
                        ...
                        case 'warn':
                            if (typeof arguments[1].newMessage === 'object') {
                                detailTable = objectToTable(
                                    elements.stackTraceTable.cloneNode(false),
                                    arguments[1].newMessage
                                );
                                msgContainer.innerHTML =
                                    elements.arrowRight + '&nbsp;' + message;
                            } else {
                        ...
                        case 'table':
                            ...
                            if (
                                method === 'table' ||
                                typeof arguments[1].newMessage === 'object'
                            ) {
                                detailTable = objectToTable(
                                    elements.stackTraceTable.cloneNode(false),
                                    arguments[1].newMessage
                        ...
                        case 'groupEnd':
                            if (method !== 'groupEnd') {
                                ...
                                msgContainer.setAttribute(
                                    'toggles',
                                    'group_' + options.groupDepth
                                );
                            } else {
                                options.groupDepth = valBetween(
                                    options.groupDepth - 1,
                                    0,
                                    99
                                );
                        ...
                        case 'timeEnd':
                            var timerName =
                                    arguments[1].newMessage || 'default',
                                now,
                                passed;
                            if (method === 'time') {
                                status.timers[timerName] = Date.now();
                                if (
                                    typeof arguments[1].original === 'function'
                                ) {
                                    arguments[1].original.apply(
                                        console,
                                        arguments[1].originalArguments
                                    ); //make sure we still call the original console.time to start the browser's console timer
                                }
                                return;
                            }
                            now = Date.now();
                    ...
                    if (
                        stackTrace &&
                        typeof stackTrace[stackTrace.length - 1] !== 'undefined'
                    ) {
                        rightContainer.appendChild(
                            setCSS(
                                getLink(
                                    stackTrace[0].url,
                                    stackTrace[0].linkText
                                ),
                                {
                                    color: '#808080',
                                }
                    ...
                    //set colors
                    setCSS(lineContainer, {
                        display: elements.buttons[method].toggled ? 'none' : '',
                    });
                ...
                originalConsole.error(
                    'mobileConsole generated an error logging this event! (type: ' +
                        typeof message +
                        ')'
                );
        ...
        function interceptConsole(method) {
            ...
            console[method] = function () {
                ...
                args.newMessage =
                    method === 'assert' ? [args[0], args[1]] : args[0];
                ...
                args.newStackTrace = formatStackTrace(
                    args.newStackTrace,
                    stackTraceOrig
                );
                if (method === 'clear') {
                    try {
                        ...
                        console.log(
                            'This browser does not allow clearing tables, the console cannot be cleared.'
                        );
                        return;
                    }
    ...
    function initCommandLine() {
        ...
        function toggleAutoComplete(show) {
            var hidden =
                (elements.autocomplete.currentStyle
                    ...
                    : window.getComputedStyle(elements.autocomplete, null)
                          .display) === 'none';
            show = typeof show === 'undefined' ? hidden : show;
            setCSS(elements.autocomplete, {
        ...
        function autoComplete(command) {
            ...
            while (i--) {
                ...
                elements.autocomplete.insertBefore(
                    row,
                    elements.autocomplete.firstChild
                );
        ...
        function setBinds() {
            if (options.browserinfo.evtLstn) {
                elements.autocomplete.addEventListener(
                    'click',
                    clickAutoComplete,
                    false
                );
                ...
                if (tgt === elements.consoleinput) {
                    if (e.key === 'Enter' || e.keyCode === 13) {
                        ...
                        if (!status.acHovered) {
                            ...
                            elements.consoleinput.value = getFromArrayById(
                                'hover'
                            ).element.innerHTML;
                            elements.consoleinput.focus();
                        }
                        ...
                        if (status.acActive) {
                            ...
                            var counter =
                                hovered === -1
                                    ? elements.acItems.length
                                    : hovered;
                            //hover new (in- or decreased number) one
                            counter = valBetween(
                                (counter += up ? 1 : -1),
                                0,
                                elements.acItems.length - 1
                            );
                            hoverAutoComplete({
                                target: elements.acItems[counter],
                                type: 'mouseover',
                            });
                        } else {
                            ...
                            history.input.commandIdx = valBetween(
                                history.input.commandIdx,
                                0,
                                hist.length
                            );
                            elements.consoleinput.value =
                                typeof hist[history.input.commandIdx] ===
                                'undefined'
                                    ? ''
                                    : decodeURI(hist[history.input.commandIdx]);
                        }
                    }
                }

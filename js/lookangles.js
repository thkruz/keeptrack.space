...
;(function () {
  ...
  satellite.currentEpoch = (currentDate) => {
    ...
    let timeOfDay =
      (currentDate.getUTCHours() * 60 + currentDate.getUTCMinutes()) / 1440
      ...
      for (let i = 0; i < looksArray.length; i++) {
        ...
        tdT.appendChild(
          document.createTextNode(
            timeManager.dateFormat(looksArray[i].time, 'isoDateTime', false),
          ),
        )
  ...
  satellite.satSensorFOV = (sat1, sat2) => {
    ...
    for (deltaLat = -60; deltaLat < 60; deltaLat += deltaLatInt) {
      lat =
        Math.max(
          Math.min(
            Math.round(satSelGeodetic.latitude * RAD2DEG) + deltaLat,
            90,
          ),
          -90,
        ) * DEG2RAD
  ...
  satellite.calculateDOPs = (satList, referenceECF, isDrawLine) => {
    ...
    for (var n = 1; n <= nsat; n++) {
      ...
      elvlist.push(elv)
      var B = [
        Math.cos((elv * Math.PI) / 180.0) * Math.sin((az * Math.PI) / 180.0),
        Math.cos((elv * Math.PI) / 180.0) * Math.cos((az * Math.PI) / 180.0),

...
(function () {
  ...
  om.kp2tle = (kp,epoch) => {
    ...
    let epochd = _dayOfYear(epoch.getUTCMonth(),
                    epoch.getUTCDate(),
                    epoch.getUTCHours(),
                    epoch.getUTCMinutes(),
                    epoch.getUTCSeconds());
    ...
    for (let r = -STEPS; r < STEPS; r++) {
      for (let a = -STEPS; a < STEPS; a++) {
        for (let m = -STEPS*2; m < STEPS*2; m++) {
          ...
          const tles = om.kp2tle(kp,epoch);
          let xError = 0;
          let yError = 0;
          let zError = 0;
          let posErrorAvg = 0;
          for (let svI = 0; svI < svs.length; svI++) {
            const eci = await _propagate(tles.tle1,tles.tle2,new Date(epoch + (svs[svI][0] - svs[0][0])));
  ...
  om.svs2analyst = async (svs) => {
    om.iod(svs).then((tles)=>{
  ...
  async function _propagate (tle1,tle2,epoch) {
  ...
  function _sv2kp (massPrimary,massSecondary,vector,massPrimaryU,massSecondaryU,vectorU,outputU,outputU2) {
  ...
  function _kp2sv(a,ec,i,w0,o0,m0,massPrimary,massSecondary,massPrimaryU,massSecondaryU,aU,vectorU) {
      let rxu,
          ryu,
          rzu = 'm'
      let vxu,
      ...
      while (diff > eps) {
          e1 =
              eca - (eca - ec * Math.sin(eca) - m0) / (1 - ec * Math.cos(eca))
  ...
  function _dayOfYear(mon, day, hr, minute, sec) {
      return (
          Math.floor((275 * mon) / 9.0) +
          day +
          ((sec / 60.0 + minute) / 60.0 + hr) / 24.0 //  ut in days

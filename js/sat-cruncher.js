    ...
    case 'satdata':
      ...
      while (i < len) {
        ...
        if (satData[i].static || satData[i].missile) {
          ...
          extra.semiMinorAxis =
            extra.semiMajorAxis * Math.sqrt(1 - Math.pow(extra.eccentricity, 2))
          extra.apogee =
            extra.semiMajorAxis * (1 + extra.eccentricity) - RADIUS_OF_EARTH
          extra.perigee =
            extra.semiMajorAxis * (1 - extra.eccentricity) - RADIUS_OF_EARTH
    ...
    if (sat.satnum) {
      ...
      if (isSunlightView) {
        ...
        theta =
          Math.acos(
            numeric.dot(
              [-satPos[i * 3], -satPos[i * 3 + 1], -satPos[i * 3 + 2]],
              [
                -satPos[i * 3] + sunECI.x,
      ...
      satVel[i * 3] =
        (6371 + satCache[i].altList[curMissivarTime + 1]) * cosLat * cosLon -
        satPos[i * 3]
      satVel[i * 3 + 1] =
        (6371 + satCache[i].altList[curMissivarTime + 1]) * cosLat * sinLon -
        satPos[i * 3 + 1]
      satVel[i * 3 + 2] =
      ...
      if (
        satellite.eciToGeodetic({ x: x, y: y, z: z }, gmst).height <= 150 &&

  ...
  (function () {
    ...
    atmosphere.init = function () {
      ...
      for (let lat = 0; lat <= atmosphere.latSegs; lat++) {
        ...
        for (let lon = 0; lon <= atmosphere.lonSegs; lon++) { // add an extra vertex for texture funness
      ...
      let vertIndex = [];
      for (let lat = 0; lat < atmosphere.latSegs; lat++) { // this is for each QUAD, not each vertex, so <
    ...
    atmosphere.update = () => {
      ...
      mat4.rotateY(atmosphere.mvMatrix, atmosphere.mvMatrix, 90 * DEG2RAD - camPitch);

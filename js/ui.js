...
;(function () {
  ...
  $(document).ready(function () {
    ...
    ;(function _menuController() {
      ...
      $('#info-overlay-content').on('click', '.watchlist-object', function (
        evt,
      ) {
      ...
      $('#settings-form').on('change', function (e) {
        ...
        var isSLMChecked = document.getElementById('settings-sat-label-mode')
          .checked
      ...
      $('#settings-form').on('submit', function (e) {
        ...
        var isRiseSetChecked = document.getElementById('settings-riseset')
          .checked
      ...
      $('#editSat').on('submit', function (e) {
        ...
        var TLE1 =
          '1 ' + scc + 'U ' + intl + ' ' + epochyr + epochday + TLE1Ending // M' and M'' are both set to 0 to put the object in a perfect stable orbit
        var TLE2 =
          '2 ' +
          scc +
          ' ' +
        ...
        $('#loading-screen').fadeIn('slow', function () {
          ...
          if (objectManager.isLaunchSiteManagerLoaded) {
            for (var launchSite in window.launchSiteManager.launchSiteList) {
              if (
                window.launchSiteManager.launchSiteList[launchSite].name ===
                launchFac
              ) {
                launchLat =
                  window.launchSiteManager.launchSiteList[launchSite].lat
                launchLon =
                  window.launchSiteManager.launchSiteList[launchSite].lon
              }
            }
          }
          if (launchLon > 180) {
            ...
            for (var incIterat = 0; incIterat <= 4; incIterat++) {
              for (var meanmoIterat = 0; meanmoIterat <= 4; meanmoIterat++) {
                for (var rascIterat = 0; rascIterat <= 4; rascIterat++) {
                  ...
                  var rascOffset =
                    -rascVariation / 2 + rascVariation * (rascIterat / 4)
                  ...
                  inc = parseFloat(
                    inc - incVariation / 2 + incVariation * (incIterat / 4),
                  ).toPrecision(7)
                  inc = inc.split('.')
                  inc[0] = inc[0].substr(-3, 3)
                  if (inc[1]) {
                  ...
                  meanmo = parseFloat(
                    meanmo -
                      ...
                      meanmo * meanmoVariation * (meanmoIterat / 4),
                  ).toPrecision(10)
    ...
    uiManager.satChng = function (row) {
      ...
      if (row === -1 && satChngTable.length === 0) {
        ...
        $.get('/analysis/satchng.json?v=' + settingsManager.versionNumber).done(
          function (resp) {
            resp = [...new Set(resp)]
            ...
            for (let i = 0; i < Math.min(satChngTable.length, 20); i++) {
              ...
              tdInc = tr.insertCell()
              tdInc.appendChild(
                document.createTextNode(satChngTable[i].inc.toFixed(2)),
              )
              tdPer = tr.insertCell()
        ...
        case 'menu-info-overlay':
          ...
          if (isInfoOverlayMenuOpen) {
            ...
            if (
              ...
              new Date(nextPassEarliestTime * 1 + 1000 * 60 * 60 * 24) <
                timeManager.now ||
              isWatchlistChanged
            ) {
              $('#loading-screen').fadeIn('slow', function () {
  ...
  function _updateNextPassOverlay(isForceUpdate) {
    ...
    if (
      ...
      for (var s = 0; s < nextPassArray.length; s++) {
        ...
        if (nextPassArray[s].time - propTime < -1000 * 60 * 5 && !satInView)
          continue
  ...
  function _updateSelectBox() {
    ...
    if (timeManager.now > lastBoxUpdateTime * 1 + updateInterval) {
      ...
      if (satellite.degreesLong(uiManager.currentTEARR.lon) >= 0) {
        $('#sat-longitude').html(
          satellite.degreesLong(uiManager.currentTEARR.lon).toFixed(3) + '°E',
        )
      } else {
        $('#sat-longitude').html(
          (satellite.degreesLong(uiManager.currentTEARR.lon) * -1).toFixed(3) +
            '°W',
        )
      }
      if (satellite.degreesLat(uiManager.currentTEARR.lat) >= 0) {
        $('#sat-latitude').html(
          satellite.degreesLat(uiManager.currentTEARR.lat).toFixed(3) + '°N',
        )
      } else {
        $('#sat-latitude').html(
          (satellite.degreesLat(uiManager.currentTEARR.lat) * -1).toFixed(3) +
            '°S',
        )
      }
      var jday = timeManager.getDayOfYear(timeManager.propTime())
      ...
      if (objectManager.isSensorManagerLoaded) {
        if (uiManager.currentTEARR.inview) {
          ...
          $('#sat-azimuth').prop(
            'title',
            'Azimuth: ' + uiManager.currentTEARR.azimuth.toFixed(0) + '°',
          )
          $('#sat-elevation').html('Out of Bounds')
          $('#sat-elevation').prop(
            'title',
            'Elevation: ' + uiManager.currentTEARR.elevation.toFixed(1) + '°',
          )
          $('#sat-range').html('Out of Bounds')
          $('#sat-range').prop(
            'title',
            'Range: ' + uiManager.currentTEARR.range.toFixed(2) + ' km',
          )
        }
      } else {
  ...
  uiManager.updateURL = () => {
    ...
    if (
      ...
      paramSlices.push(
        'sat=' + satSet.getSatExtraOnly(objectManager.selectedSat).SCC_NUM,
      )
    }
    ...
    if (timeManager.propOffset < -1000 || timeManager.propOffset > 1000) {
      paramSlices.push(
        'date=' +
          (timeManager.propRealTime + timeManager.propOffset).toString(),
      )
    }
  ...
  uiManager.updateMap = function () {
    ...
    for (var i = 1; i <= 50; i++) {
      map = mapManager.braun(
        { lon: satellite.map(sat, i).lon, lat: satellite.map(sat, i).lat },
        { meridian: 0, latLimit: 90 },
      )

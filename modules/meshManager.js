;(function () {
  ...
  meshManager.drawObject = (model, pMatrix, camMatrix) => {
    ...
    mat4.translate(
      mvMatrix,
      mvMatrix,
      vec3.fromValues(model.position.x, model.position.y, model.position.z),
    )
  ...
  function initShaders() {
    ...
    meshManager.shaderProgram.enableVertexAttribArrays = function (model) {
      for (const attrName in attrs) {
        ...
        meshManager.shaderProgram.attrIndices[attrName] = gl.getAttribLocation(
          meshManager.shaderProgram,
          attrName,
        )
        if (meshManager.shaderProgram.attrIndices[attrName] != -1) {
  ...
  function initBuffers() {
    ...
    for (var mesh in meshManager.meshes) {
      ...
      var indexData = meshManager.meshes[mesh].makeIndexBufferDataForMaterials(

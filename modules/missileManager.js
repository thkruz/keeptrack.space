...
;(function () {
  ...
  missileManager.Missile = function (
    ...
    var minAltitudeTrue =
      minAltitude * (Math.min(3, MaxMissileRange / (ArcLength / 1000)) / 2)
  ...
  missileManager.asat = (
    ...
    let TargetLongitude =
      CurrentLongitude >= 0 ? CurrentLongitude - 180 : CurrentLongitude + 180
    let timeInFlight
  ...
  function _CoordinateCalculator(
    ...
    var Alpha1 = Math.atan2(
      Math.sin(Lambda12),
      Math.cos(Phi1) * Math.tan(Phi2) - Math.sin(Phi1) * Math.cos(Lambda12),
    ) // (Rad)
    ...
    for (var i = 0; i <= 2400; i++) {
      ...
      var Lambda =
        ((Lambdao +
          Math.atan2(Math.sin(Alphao) * Math.sin(Sigma), Math.cos(Sigma))) *
          180) /
        Math.PI // (Degrees)

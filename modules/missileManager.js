...
(function () {
    ...
    missileManager.ukSLBM = [
        56.066111,
        -4.8175,
        'Vanguard Sub (Trident II)',
        12000,
        56.066111,
        -4.8175,
        'HMNB Clyde (Trident II)',
        12000,
    ];
            ...
            for (var i = 0; i < missileArray.length; i++) {
                ...
                if (MissileObject) {
                    ...
                    orbitManager.updateOrbitBuffer(
                        ...
                        null,
                        true,
                        MissileObject.latList,
                        MissileObject.lonList,
                        MissileObject.altList,
                        MissileObject.startTime
                    );
                }
            }
            missileManager.missileArray = missileArray;
        ...
        if (RaidType === 'Russia') {
            ...
            for (a = 0; a < missileManager.RussianICBM.length / 4; a++) {
                ...
                for (i = 0; i < USATargets.length / 2; i++) {
                    ...
                    if (
                        missileManager.RussianICBM[a * 4 + 3] !== 8300 &&
                        missilesLaunched > 19
                    )
                        continue; // 20 missiles per site
                    if (
                        missileManager.RussianICBM[a * 4 + 3] === 8300 &&
                        missilesLaunched > 11
                    )
                        continue; // 12 missiles per sub
                    if (Math.random() < 0.3) {
                ...
                if (
                    (missileManager.RussianICBM[a * 4 + 3] !== 8300 &&
                        missilesLaunched <= 18 &&
                        i >= USATargets.length / 2) || // If less than 25 missiles launched redo that brigade
                    (missileManager.RussianICBM[a * 4 + 3] === 8300 &&
                        missilesLaunched <= 10 &&
                        i >= USATargets.length / 2)
                ) {
            ...
            for (a = 0; a < missileManager.UsaICBM.length / 4; a++) {
                ...
                if (RaidType === 'USA2Russia') {
                    for (
                        i = 0;
                        i < missileManager.RussianICBM.length / 4;
                        i++
                    ) {
                    ...
                    for (
                        i = 0;
                        i < missileManager.NorthKoreanBM.length / 4;
                        i++
                    ) {
    ...
    missileManager.Missile = function (
        ...
        if (CurrentLatitude > 90 || CurrentLatitude < -90) {
            console.error(
                'Error: Current Latitude must be between 90 and -90 degrees'
            );
            return 0;
        }
        if (CurrentLongitude > 180 || CurrentLongitude < -180) {
            console.error(
                'Error: Current Longitude must be between 180 and -180 degrees'
            );
            return 0;
        }
        ...
        if (parseFloat(NumberWarheads) % 1 > 0) {
            console.error(
                'Error: The number of warheads must be a whole number'
            );
            return 0;
        }
        ...
        calculatedCoordinates = _CoordinateCalculator(
            CurrentLatitude,
            CurrentLongitude,
            TargetLatitude,
            TargetLongitude
        ...
        var minAltitudeTrue =
            minAltitude *
            (Math.min(3, MaxMissileRange / (ArcLength / 1000)) / 2);
        ...
        var RocketCasingVolume =
            0.25 *
            Math.PI *
            Length *
            (Math.pow(Diameter, 2) - Math.pow(Diameter - Thickness, 2)); // (m^3)
        var RocketCasingMass1 = RocketCasingDensity * RocketCasingVolume; // (kg)
        var RocketCasingMass2 =
            RocketCasingDensity *
            0.25 *
            Math.PI *
            (Length * 0.4937) *
            (Math.pow(Diameter, 2) - Math.pow(Diameter - Thickness, 2)); // (kg)
        var RocketCasingMass3 =
            RocketCasingDensity *
            0.25 *
            Math.PI *
            (Length * 0.157) *
            (Math.pow(Diameter * 0.75, 2) -
                (Diameter * 0.75 - Math.pow(Thickness / 2), 2)); // (kg)
        ...
        var FuelArea2 =
            0.25 * Math.PI * Math.pow(Diameter * 0.75 - Thickness, 2); // (m^2)
        var FuelVolume =
            FuelArea1 * (Length * 0.651) + FuelArea2 * (Length * 0.178); // (m^3)
        ...
        var AngleCoefficient = _Bisection(
            FuelArea1,
            FuelArea2,
            FuelMass,
            FuelVolume,
        ...
        while (FuelMass / FuelDensity / FuelVolume > 0.4 && Altitude >= 0) {
            var iterationFunOutput = _IterationFun(
                FuelArea1,
                FuelMass,
                RocketArea,
                Altitude,
            ...
            AdjacentList.push(
                Math.cos(ArcDistance / EarthRadius) * (Altitude + EarthRadius)
            );
            OppositeList.push(
                Math.sin(ArcDistance / EarthRadius) * (Altitude + EarthRadius)
            );
            ...
            for (i = 0; i < EstDistanceList.length; i++) {
                if (
                    EstDistanceList[i] <= Distance / 1000 &&
                    !(EstDistanceList[i + 1] <= Distance / 1000)
                ) {
        ...
        for (i = 0; i < drdtList.length; i++) {
            if (drdtList[i] === MaxVerticalVelocity)
                var MaxVerticalVelocityPossition = i;
        }
        var AverageAngularVelocity = 0;
        for (i = 0; i < dthetadtList.length; i++) {
            if (dthetadtList[i] === MaxAngularVelocity)
                var MaxAngularVelocityPossition = i;
            AverageAngularVelocity += dthetadtList[i];
        }
        for (i = 0; i < Exitdr2dtList.length; i++) {
            if (Exitdr2dtList[i] === MaxVerticalAcceleration)
                var MaxVerticalAccelerationPossition = i;
        }
        for (i = 0; i < Exitdtheta2dtList.length; i++) {
            if (Exitdtheta2dtList[i] === MaxAngularAcceleration)
                var MaxAngularAccelerationPossition = i;
        }
    ...
    missileManager.asat = (
        ...
        let TargetLongitude =
            CurrentLongitude >= 0
                ...
                : CurrentLongitude + 180;
        let timeInFlight;
        timeInFlight = missileManager.asatPreFlight(
            CurrentLatitude,
            CurrentLongitude,
            TargetLatitude,
            TargetLongitude,
        ...
        let propOffset = timeManager.propTimeCheck(
            timeInFlight * 1000,
            startTime
        );
        console.log(propOffset);
        let satTEARR2 = satellite.getTEARR(
            sat,
        ...
        TargetLongitude =
            TargetLongitude > 180 ? TargetLongitude - 360 : TargetLongitude;
        TargetLongitude =
            TargetLongitude < -180 ? TargetLongitude + 360 : TargetLongitude;
        ...
        console.log(propOffset);
        let satTEARR3 = satellite.getTEARR(
            sat,
            sensorManager.sensorList.COD,
            propOffset
        );
    ...
    missileManager.getMissileTEARR = function (missile, sensor) {
        ...
        x =
            (RADIUS_OF_EARTH + missile.altList[curMissileTime]) *
            cosLat *
            cosLon;
        y =
            (RADIUS_OF_EARTH + missile.altList[curMissileTime]) *
    ...
    function _CD(M) {
        ...
        if (M < 1.1875)
            return (
                -0.329086061307 +
                2.30117394072 * M +
                -4.06597222013 * Math.pow(M, 2) +
                3.01851851676 * Math.pow(M, 3) +
                -0.666666666129 * Math.pow(M, 4)
            );
        if (M < 1.625)
            return (
                0.10937644721 +
                -4.61979595244 * M +
                9.72917139612 * Math.pow(M, 2) +
                -6.33333563852 * Math.pow(M, 3) +
                1.33333375211 * Math.pow(M, 4)
            );
        if (M < 3.625)
            return (
                0.97916002909 +
                -0.540978181863 * M +
                0.125235817144 * Math.pow(M, 2) +
                -0.00666103733277 * Math.pow(M, 3) +
                -0.000558009790208 * Math.pow(M, 4)
            );
    ...
    function _CoordinateCalculator(
        ...
        var Lambda01 = Math.atan2(
            Math.sin(Alphao) * Math.sin(DeltaSigma01),
            Math.cos(DeltaSigma01)
        ); // (Rad)
        ...
        for (var i = 0; i <= 2400; i++) {
            var Sigma =
                DeltaSigma01 + (i * (DeltaSigma02 - DeltaSigma01)) / 2000; // (Rad)
            var Phi =
                (Math.asin(Math.cos(Alphao) * Math.sin(Sigma)) * 180) / Math.PI; // (Degrees)
            var Lambda =
                ((Lambdao +
                    Math.atan2(
                        Math.sin(Alphao) * Math.sin(Sigma),
                        Math.cos(Sigma)
                    )) *
                    180) /
                Math.PI; // (Degrees)
    ...
    function _IterationFun(
        ...
        var DragForce =
            (1 / 2) *
            AirDensity *
            (Math.pow(drdt, 2) + Math.pow(dthetadt, 2)) *
            RocketArea *
            cD; // (N)
        ...
        var dr2dt =
            (Thrust * Math.sin(ThrustAngle) -
                DragForce * Math.sin(DragAngle) -
                WeightForce) /
                RocketMass +
            Radius * Math.pow(dthetadt / Radius, 2); // (m/s^2)
        ...
        var dtheta2dt =
            (Thrust * Math.cos(ThrustAngle) - DragForce * Math.cos(DragAngle)) /
                RocketMass -
            2 * drdt * (dthetadt / Radius); // (m/s^2)

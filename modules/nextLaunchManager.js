...
$.get('https://launchlibrary.net/1.4/launch/next/20')
  .done(function (resp) {
  ...
  function _initTable () {
    ...
    for (let i = 0; i < launchList.length; i++) {
      ...
      if (launchList[i].windowStart <= (Date.now()-(1000*60*60*24))) {
        ...
        timeText = timeManager.dateFormat(launchList[i].windowStart, 'isoDateTime', true) + ' UTC';
      }
      ...
      // Mission Name Text
      let nameText = (typeof launchList[i].missionName != 'undefined') ?
                     launchList[i].missionName : 'Unknown';

...
satCommManager.showLinks = async function (group) {
  ...
  for (let i = 0; i < satlist.length; i++) {
    for (let j = 0; j < satlist.length; j++) {
      if (i !== j) {
        ...
        var theta = Math.acos(numeric.dot([-sat1.position.x,-sat1.position.y,-sat1.position.z],[-sat1.position.x + sat2.position.x,-sat1.position.y + sat2.position.y,-sat1.position.z + sat2.position.z])/(Math.sqrt(Math.pow(-sat1.position.x, 2) + Math.pow(-sat1.position.y, 2) + Math.pow(-sat1.position.z, 2))*Math.sqrt(Math.pow(-sat1.position.x + sat2.position.x, 2) + Math.pow(-sat1.position.y + sat2.position.y, 2) + Math.pow(-sat1.position.z + sat2.position.z, 2)))) * RAD2DEG;
        if (theta < minTheta) {
        } else {
  ...
  for (let i = 0; i < userlist.length; i++) {
    ...
    if (typeof bestSat.id !== 'undefined') {
      debugDrawLine('sat3',[bestSat.id,satSet.getIdFromSensorName(user.name)],[0, 1, 0.6, 1]);
    }
  }
};

// Milstar and AEHF
satCommManager.AEHF = [22988, 23712, 26715, 27168, 27711, 36868,
                       38254, 39256, 43651, 44481];
satCommManager.DSCS = [23628, 25019, 26052, 27691, 27875, 22915];
satCommManager.WGS = [32258, 34713, 36108, 38070, 39168, 39222, 40746, 41879,
                      42075, 44071];
satCommManager.Iridium = [24841, 24870, 41917, 41918,41919,41920,41921,41922,
                          41923,41924,41925,41926,42803,42804,42805,42806,42807,
                          42808,42809,42810,42811,42812,43569,43570,43571,43572,

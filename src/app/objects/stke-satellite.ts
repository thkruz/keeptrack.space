...
/**
 * Interface for STK.e file header information
 */
export interface StkeHeader {
  START_TIME: Date;
  STOP_TIME: Date;
...
/**
 * Interface for STK.e metadata
 */
export interface StkeMetadata {
  OBJECT_NAME: string;
  OBJECT_ID?: string;
  CENTER_NAME: 'EARTH' | 'MARS BARYCENTER' | 'MOON';
  REF_FRAME: string;
  TIME_SYSTEM: string;
  START_TIME: string;
  STOP_TIME: string;
...
/**
 * Interface for STK.e data block
 */
export interface StkeDataBlock {
  metadata: StkeMetadata;
  ephemeris: J2000[];
}

/**
 * Interface for parsed STK.e file
 */
export interface ParsedStke {
  header: StkeHeader;
  dataBlocks: StkeDataBlock[];
}

/**
 * StkeSatellite class for maintaining STK.e ephemeris data
 * Similar to OemSatellite but for STK.e format
 */
export class StkeSatellite extends BaseObject {
  header: StkeHeader;
  stkeDataBlocks: StkeDataBlock[];
  ...
  constructor(stke: ParsedStke) {
    const lastDataBlock = stke.dataBlocks[stke.dataBlocks.length - 1];

    if (!lastDataBlock) {
      throw new Error('No data blocks found in STK.e file.');
    }

    if (lastDataBlock.ephemeris.length === 0) {
      throw new Error('No state vectors found in the last data block of the STK.e file.');
    }
    ...
    super({
      id: 0,
      name: stke.dataBlocks[0].metadata.OBJECT_NAME ?? 'Unnamed Satellite',
    ...
    switch (stke.dataBlocks[0].metadata.CENTER_NAME) {
      case 'MARS BARYCENTER':
        this.centerBody = SolarBody.Mars;
        break;
      case 'MOON':
        this.centerBody = SolarBody.Moon;
        break;
      case 'EARTH':
      default:
    ...
    this.stkeDataBlocks = stke.dataBlocks;
    this.lagrangeInterpolator = LagrangeInterpolator.fromEphemeris(
      this.stkeDataBlocks.flatMap((block) => block.ephemeris),
      this.stkeDataBlocks[0].metadata.INTERPOLATION_DEGREE ?? 7,
    );
    this.source = 'STK.e Import';
    this.header = stke.header;
  ...
  updatePosAndVel(simTime: Seconds): [number, number, number, number, number, number] | null {
    if (!this?.stkeDataBlocks || this.stkeDataBlocks.length === 0) {
      return null;
    }
    ...
    dt = (simTime - this.stkeDataBlocks[this.dataBlockIdx].ephemeris[this.stateVectorIdx].epoch.posix) as Seconds;

    const currentSv: TEME = this.stkeDataBlocks[this.dataBlockIdx].ephemeris[this.stateVectorIdx].toTEME();
    ...
    const nextSv: TEME =
      this.stkeDataBlocks[this.dataBlockIdx].ephemeris[this.stateVectorIdx + 1]?.toTEME() ??
      this.stkeDataBlocks[this.dataBlockIdx + 1]?.ephemeris[0]?.toTEME() ??
      currentSv;
  ...
  private findStateVectorTime_(simTime: number): void {
    ...
    if (startBlock < this.stkeDataBlocks.length) {
      const vectors = this.stkeDataBlocks[startBlock].ephemeris;
    ...
    let searchStart = 0;
    let searchEnd = this.stkeDataBlocks.length;

    if (startBlock < this.stkeDataBlocks.length) {
      const currentBlockVectors = this.stkeDataBlocks[startBlock].ephemeris;
    ...
    for (let i = searchStart; i < searchEnd; i++) {
      const vectors = this.stkeDataBlocks[i].ephemeris;
      ...
      if (i < this.stkeDataBlocks.length - 1 &&
  ...
  private generateOrbitPath_(): Float32Array {
    for (const block of this.stkeDataBlocks) {
  ...
  private getSegmentsFromCache_(segments: number) {
    ...
    for (let i = 0; i < this.dataBlockIdx; i++) {
      currentIndex += this.stkeDataBlocks[i].ephemeris.length;
    }

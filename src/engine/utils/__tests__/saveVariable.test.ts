...
import { copyTsvToClipboard, getCircularReplacer, saveCsv, saveVariable, saveXlsx } from '@app/engine/utils/saveVariable';
...
describe('code_snippet', () => {
  ...
  it('test_copy_tsv_to_clipboard', async () => {
    const items = [
      { a: 1, b: 2 },
      { a: 3, b: 4 },
    ];
    ...
    await copyTsvToClipboard(items);

    expect(writeTextSpy).toHaveBeenCalledTimes(1);
    const tsvOutput = writeTextSpy.mock.calls[0][0] as string;

    // Verify tab-separated format
    expect(tsvOutput).toContain('\t');
    expect(tsvOutput).not.toContain(',');
    // PapaParse uses \r\n line endings
  ...
  // Tests that copyTsvToClipboard handles clipboard errors gracefully
  it('test_copy_tsv_to_clipboard_error', async () => {
    const items = [{ a: 1 }];
    ...
    disableConsoleErrors();
    await expect(copyTsvToClipboard(items)).resolves.not.toThrow();
    enableConsoleErrors();
  });
});

...
ColorScheme.prototype.calculateColorBuffers = function (isForceRecolor) {
    ...
    if (
        !isForceRecolor &&
        now - lastCalculation < settingsManager.reColorMinimumTime &&
        lastCalculation !== 0
    ) {
        return {
    ...
    if (
        ...
        numSats -= settingsManager.maxFieldOfViewMarkers;
...
var sat, color;
ColorScheme.init = function (cameraManager) {
    ColorScheme.default = new ColorScheme(function (sat) {
        if (sat.static && sat.type === 'Star') {
            if (sat.vmag >= 4.7 && ColorScheme.objectTypeFlags.starLow) {
                ...
                sat.vmag >= 3.5 &&
                sat.vmag < 4.7 &&
                ColorScheme.objectTypeFlags.starMed
            ) {
                return {
                ...
                sat.vmag < 3.5 &&
                ColorScheme.objectTypeFlags.starHi
            ) {
                return {
        ...
        if (sat.static &&
            (ColorScheme.objectTypeFlags.sensor === false || cameraManager.cameraType.current === cameraManager.cameraType.planetarium)) {
            return {
                color: colorTheme.deselected,
                pickable: false,
            };
    ...
    ColorScheme.apogee = new ColorScheme(function (sat) {
        ...
        colorTheme.apogeeGradient = [
            1.0 - colorTheme.gradientAmt,
            colorTheme.gradientAmt,
            0.0,
            1.0,
        ];
        return {
    ...
    ColorScheme.smallsats = new ColorScheme(function (sat) {
        if (
            sat.OT === 1 &&
            ColorScheme.objectTypeFlags.satSmall === false
        ) {
            return {
                color: colorTheme.deselected,
                pickable: false,
    ...
    ColorScheme.ageOfElset = new ColorScheme(function (sat) {
        ...
        if (sat.TLE1.substr(18, 2) === now) {
            ...
            daysold =
                jday -
                sat.TLE1.substr(20, 3) +
                sat.TLE1.substr(17, 2) * 365;
        }
        ...
        if (
            daysold >= 3 &&
            daysold < 14 &&
            ColorScheme.objectTypeFlags.ageMed
        ) {
            return {
        ...
        if (
            daysold >= 14 &&
            daysold < 60 &&
            ColorScheme.objectTypeFlags.ageOld
        ) {
            return {
    ...
    ColorScheme.lostobjects = new ColorScheme(function (sat) {
        ...
        if (
            pe > satellite.obsmaxrange ||
            daysold < settingsManager.daysUntilObjectLost
        ) {
            return {
                color: colorTheme.transparent,
                pickable: false,

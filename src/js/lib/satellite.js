(function (root, factory) {
    ...
    define('constants', [], function () {
        ...
        var pi = Math.PI,
            ...
            xke =
                60.0 /
                Math.sqrt((earthRadius * earthRadius * earthRadius) / mu),
            j2 = 0.00108262998905,
            j3 = -0.00000253215306;
    ...
    ], function (topocentric, topocentricToLookAngles) {
        'use strict';

        return function (observerCoordsEcf, satelliteCoordsEcf) {
            var topocentricCoords = topocentric(
                observerCoordsEcf,
                satelliteCoordsEcf
            );
    ...
    define('coordinate-transforms/eci-to-ecf', [], function () {
        ...
        return function (eciCoords, gmst) {
            ...
            var Y =
                eciCoords.x * -Math.sin(gmst) + eciCoords.y * Math.cos(gmst);
            var Z = eciCoords.z;
            return { x: X, y: Y, z: Z };
        };
    });
    ...
    define('coordinate-transforms/eci-to-geodetic', [], function () {
        ...
        return function (eciCoords, gmst) {
            ...
            var b = 6356.7523142;
            var R = Math.sqrt(
                eciCoords.x * eciCoords.x + eciCoords.y * eciCoords.y
            );
            var f = (a - b) / a;
            var e2 = 2 * f - f * f;
            ...
            while (k < kmax) {
                C =
                    1 /
                    Math.sqrt(
                        1 - e2 * (Math.sin(latitude) * Math.sin(latitude))
                    );
                latitude = Math.atan2(
                    eciCoords.z + a * C * e2 * Math.sin(latitude),
                    R
                );
                k += 1;
            }
    ...
    define('gstime/gstime', ['../constants'], function (constants) {
        'use strict';
    ...
    define('dpper', ['./constants'], function (constants) {
        'use strict';
    ...
    define('dspace', ['./constants'], function (constants) {
        'use strict';
    ...
    define('sgp4', [
        ...
        './dpper',
        './dspace',
    ], function (constants, dpper, dspace) {
        'use strict';
         ...
         *    vallado, crawford, hujsak, kelso  2006
         ----------------------------------------------------------------------------*/
            ...
            if (pl < 0.0) {
                ...
                var mvt =
                    rdotl -
                    (nm * temp1 * satrec.x1mth2 * sin2u) / constants.xke;
                rvdot =
                    rvdotl +
                    (nm *
                        temp1 *
                        (satrec.x1mth2 * cos2u + 1.5 * satrec.con41)) /
                        constants.xke;
    ...
    define('dscom', ['./constants'], function (constants) {
        'use strict';
         ...
         *    vallado, crawford, hujsak, kelso  2006
         ----------------------------------------------------------------------------*/
            ...
            xnodce = (4.523602 - 9.2422029e-4 * day) % constants.twoPi;
            ...
            zmol = (4.7199672 + 0.2299715 * day - gam) % constants.twoPi;
    ...
    define('dsinit', ['./constants'], function (constants) {
        'use strict';
         ...
         ----------------------------------------------------------------------------*/
            ...
            if (irez !== 0) {
                ...
                if (irez === 2) {
                    ...
                    xlamo =
                        (mo + nodeo + nodeo - theta - theta) % constants.twoPi;
                ...
                if (irez === 1) {
                    ...
                    f311 =
                        0.9375 * sinim * sinim * (1.0 + 3.0 * cosim) -
                        0.75 * (1.0 + cosim);
                    f330 = 1.0 + cosim;
    ...
    define('initl', [
         ...
         *    vallado, crawford, hujsak, kelso  2006
         ----------------------------------------------------------------------------*/
            ...
            d1 =
                (0.75 * constants.j2 * (3.0 * cosio2 - 1.0)) /
                (rteosq * omeosq);
    ...
    define('sgp4init', [
        ...
        './initl',
        './sgp4',
    ], function (constants, dpper, dscom, dsinit, initl, sgp4) {
        'use strict';
    ...
    define('propagate/twoline2satrec', [
        ...
        '../gstime/jday',
        '../sgp4init',
    ], function (constants, days2mdhms, jday, sgp4init) {
        'use strict';

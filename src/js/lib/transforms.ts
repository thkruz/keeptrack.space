...
export const normalizeAngle = (angle: Radians): Radians => {
  ...
  if (normalizedAngle > TAU / 2) normalizedAngle -= TAU;
  if (normalizedAngle < -TAU / 2) normalizedAngle += TAU;
...
export const alt2zoom = (alt: Kilometers, minZoomDistance: Kilometers, maxZoomDistance: Kilometers, minDistanceFromSatellite: Kilometers): number => {
  if (minZoomDistance > maxZoomDistance) throw new Error('minZoomDistance must be less than maxZoomDistance');
...
export const getDayOfYear = (date = new Date()): number => {
  ...
  if (mn > 1 && isLeapYear_(date)) dayOfYear++;
  return dayOfYear;
};
...
export const jday = (year: number, month: number, day: number, hour: number, minute: number, second: number): number => {
  ...
  if (year < 0 || month < 1 || day < 0 || hour < 0 || minute < 0 || second < 0) throw new Error('Invalid negative value');
  // Validate month
  if (month > 12) throw new Error('Invalid month value');
  // Validate day
  if (day > 31) throw new Error('Invalid day value');
  // Validate hour
  if (hour > 23) throw new Error('Invalid hour value');
  // Validate minute
  if (minute > 59) throw new Error('Invalid minute value');
  // Validate second
  if (second > 60) throw new Error('Invalid second value');
  return (
    367.0 * year -
...
export const dateFromJday = (year: number, day: number): Date => {
  if (year < 0) throw new Error('Invalid negative value');
  if (day < 1 || day > 366) throw new Error('Invalid day value');

  const date = new Date(Date.UTC(year, 0)); // initialize a date in `year-01-01` in UTC
  if (isLeapYear_(date)) {
    if (day > 366) throw new Error('Invalid day value');
  } else {
    if (day > 365) throw new Error('Invalid day value');
  }

  ...
  'use strict';
    ...
    satellite.getTEARR = (sat, sensor, propTime) => {
        if (!objectManager.isSensorManagerLoaded)
            throw new Error('satellite.getTEARR requires sensorManager');
    ...
    satellite.calculateLookAngles = (sat, sensor, propOffset) => {
        ...
        for (
            ...
            if (lookanglesTable.length <= 5000) {
                // Maximum of 1500 lines in the look angles table
                let lookanglesRow = _propagate(propTempOffset, satrec);
    ...
    satellite.findBestPasses = (sats, sensor) => {
        ...
        uiManager.saveCsv(sortedTableSatTimes, 'bestSatTimes');
    };
    ...
    //   let propLength = 1000 * 60 * 1440; //ms
      ...
      uiManager.saveVariable(trainData,'train-data.json');
      uiManager.saveVariable(trainTarget,'train-target.json');
      uiManager.saveVariable(testData,'test-data.json');
      uiManager.saveVariable(testTarget,'test-target.json');
    };
    ...
    //   let tt = 0;
      ...
      uiManager.saveVariable(satEciDataArray,'metObs.json');
    };
    ...
    satellite.getDOPsTable = (lat, lon, alt) => {
        ...
        for (let t = 0; t < 1440; t++) {
            ...
            let dops = satellite.getDOPs(lat, lon, alt, false, now);
    ...
    satellite.getDOPs = (lat, lon, alt, isDrawLine, propTime) => {
        ...
        if (typeof groups.GPSGroup == 'undefined') {
            groups.GPSGroup = new groups.SatGroup('nameRegex', /NAVSTAR/ui);
        }

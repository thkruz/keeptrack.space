...
export const uiManagerInit = (): void => {
  ...
  document.getElementById('editSat-open').addEventListener('click', function () {
    document.getElementById('editSat-file').click();
  });

  document.getElementById('editSat-file').addEventListener('change', function (evt: Event) {
    if (!window.FileReader) return; // Browser is not compatible
    doReaderActions(evt);
    evt.preventDefault();
  });

  document.getElementById('es-error').addEventListener('click', function () {
  ...
  if (iconName === 'menu-editSat') {
    if (isEditSatMenuOpen) {
      ...
      if (objectManager.selectedSat !== -1) {
        ...
        uiManager.hideSideMenus();
        slideInRight(document.getElementById('editSat-menu'), 1000);
        document.getElementById('menu-editSat').classList.add('bmenu-item-selected');
        isEditSatMenuOpen = true;

        const sat = satSet.getSatExtraOnly(objectManager.selectedSat);
        (<HTMLInputElement>document.getElementById('es-scc')).value = sat.sccNum;
        ...
        (<HTMLInputElement>document.getElementById('es-inc')).value = stringPad.pad0(inc, 8);
        (<HTMLInputElement>document.getElementById('es-year')).value = sat.TLE1.substr(18, 2);
        (<HTMLInputElement>document.getElementById('es-day')).value = sat.TLE1.substr(20, 12);
        (<HTMLInputElement>document.getElementById('es-meanmo')).value = sat.TLE2.substr(52, 11);
        ...
        (<HTMLInputElement>document.getElementById('es-rasc')).value = stringPad.pad0(rasc, 8);
        (<HTMLInputElement>document.getElementById('es-ecen')).value = sat.eccentricity.toPrecision(7).substr(2, 7);
        ...
        (<HTMLInputElement>document.getElementById('es-argPe')).value = stringPad.pad0(argPe, 8);
        (<HTMLInputElement>document.getElementById('es-meana')).value = sat.TLE2.substr(44 - 1, 7 + 1);
      } else {
...
export const hideSideMenus = () => {
  slideOutLeft(document.getElementById('editSat-menu'), 1000);
  document.getElementById('menu-editSat').classList.remove('bmenu-item-selected');
  isEditSatMenuOpen = false;
};
...
export const editSatNewTleClickFadeIn = () => {
  ...
  try {
    ...
    const satId = satSet.getIdFromObjNum(parseInt((<HTMLInputElement>document.getElementById('es-scc')).value));
...
export const editSatSubmit = (e: Event) => {
  ...
  let inc = <StringifiedNubmer>(<HTMLInputElement>document.getElementById('es-inc')).value;
  let meanmo = <StringifiedNubmer>(<HTMLInputElement>document.getElementById('es-meanmo')).value;
  let rasc = <StringifiedNubmer>(<HTMLInputElement>document.getElementById('es-rasc')).value;
  const ecen = (<HTMLInputElement>document.getElementById('es-ecen')).value;
  let argPe = <StringifiedNubmer>(<HTMLInputElement>document.getElementById('es-argPe')).value;
  let meana = <StringifiedNubmer>(<HTMLInputElement>document.getElementById('es-meana')).value;
  const epochyr = (<HTMLInputElement>document.getElementById('es-year')).value;
  const epochday = (<HTMLInputElement>document.getElementById('es-day')).value;

...
import { saveCsv, saveVariable, stringPad } from '@app/js/lib/helpers';
import { EciVector, sunObject, TearrData } from '@app/types/types';
import $ from 'jquery';
...
export const currentEpoch = (currentDate: any): [string, string] => {
  const { timeManager } = keepTrackApi.programs;

  const currentDateObj = new Date(currentDate);
  let epochYear: string = currentDateObj.getUTCFullYear().toString();
  epochYear = epochYear.substr(2, 2);
  let epochDay: any = timeManager.getDayOfYear(currentDateObj);
  let timeOfDay = (currentDateObj.getUTCHours() * 60 + currentDateObj.getUTCMinutes()) / 1440;
  epochDay = stringPad.pad0((epochDay + timeOfDay).toFixed(8), 12);
  // epochDay = stringPad.pad0(epochDay, 12);
  return [epochYear, epochDay];
};
export const distance = (hoverSat: SatObject, selectedSat: SatObject) => {
...
export const setobs = (sensor: any) => {
...
export const setTEARR = (currentTEARR: TearrData) => {
  satellite.currentTEARR = currentTEARR;
};
export const getTEARR = (sat?: SatObject, sensor?: SensorObject, propTime?: Date) => {
  const { timeManager, sensorManager } = keepTrackApi.programs;

  let currentTEARR: any = {}; // Most current TEARR data that is set in satellite object and returned.
...
export const nextpassList = (satArray: SatObject[]) => {
  let nextPassArray = [];
...
export const nextpass = (sat: SatObject, sensor?: SensorObject, searchLength?: number, interval?: number) => {
  const { timeManager, sensorManager } = keepTrackApi.programs;
...
export const nextNpasses = (sat: SatObject, sensor: SensorObject, searchLength: number, interval: number, numPasses: number) => {
  const { timeManager, sensorManager } = keepTrackApi.programs;
...
export const getlookangles = (sat: SatObject) => {
  const { timeManager, sensorManager } = keepTrackApi.programs;
...
export const getlookanglesMultiSite = (sat) => {
  const { timeManager, sensorManager } = keepTrackApi.programs;
  ...
  // Populate the Side Menu
  (function _populateSideMenu() {
    var tbl = <HTMLTableElement>document.getElementById('looksmultisite'); // Identify the table to update
    tbl.innerHTML = ''; // Clear the table from old object data
    var tr = tbl.insertRow();
    var tdT = tr.insertCell();
    tdT.appendChild(document.createTextNode('Time'));
...
/* istanbul ignore next */
export const findBestPasses = (sats, sensor) => {
  const { satSet } = keepTrackApi.programs;
...
export const eci2Rae = (now, eci, sensor) => {
  now = new Date(now);
  let j = _jday(
    now.getUTCFullYear(),
    now.getUTCMonth() + 1, // NOTE:, this function requires months in rng 1-12.
...
export const getEci = (sat, propTime) => {
  let j = _jday(
    propTime.getUTCFullYear(),
    propTime.getUTCMonth() + 1, // NOTE:, this function requires months in rng 1-12.
    propTime.getUTCDate(),
...
/* istanbul ignore next */
export const findNearbyObjectsByOrbit = (sat) => {
  const { satSet } = keepTrackApi.programs;
...
export const getDops = (lat: number, lon: number, alt?: number, propTime?: Date) => {
  ...
  try {
    ...
    for (var i = 0; i < groupsManager.GPSGroup.sats.length; i++) {
      let sat = satSet.getSat(groupsManager.GPSGroup.sats[i].satId);
...
export const getSunDirection = (jd: number): EciVector => {
  const n = jd - 2451545;
  ...
  const x = 1000000 * Math.cos(ecLon * DEG2RAD);
  const y = 1000000 * Math.cos(ob * DEG2RAD) * Math.sin(ecLon * DEG2RAD);
  const z = 1000000 * Math.sin(ob * DEG2RAD) * Math.sin(ecLon * DEG2RAD);

  return [x, y, z];
};
export const getSunTimes = (sat: SatObject, sensor?: SensorObject, searchLength?: number, interval?: number) => {
  const { timeManager, sensorManager } = keepTrackApi.programs;
...
export const getLlaTimeView = (propOffset: number, sat: SatObject) => {
  ...
  let gpos = satellite.eciToGeodetic(positionEci.position, gmst);
  let lat = satellite.degreesLat(gpos.lat);
  let lon = satellite.degreesLong(gpos.lon);
...
export const map = (sat: SatObject, i: number): { time: string; lat: number; lon: number; inView: number } => {
  const { timeManager } = keepTrackApi.programs;
...
export const createTle = (sat, inc, meanmo, rasc, argPe, meana, ecen, epochyr, epochday, intl, scc) => {

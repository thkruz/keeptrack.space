...
import { Singletons, UserSettings } from '@app/js/interfaces';
...
export class Earth {
  ...
  public static getSrcHiResDay(settings: UserSettings): string {
    ...
    if (settings.politicalImages) src = `${settings.installDirectory}textures/political8k.jpg`;
  ...
  public draw(pMatrix: mat4, tgtBuffer: WebGLFramebuffer, altNightTexBind?: (gl: WebGL2RenderingContext, textureNight: WebGLTexture, textureDay: WebGLTexture) => void) {
    ...
    gl.disableVertexAttribArray(this.attribs_.aTexCoord);
    gl.disableVertexAttribArray(this.attribs_.aVertexPosition);
    gl.disableVertexAttribArray(this.attribs_.aVertexNormal);

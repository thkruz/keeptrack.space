...
export class KeyboardInput {
    ...
    keyUpHandler(evt: KeyboardEvent) {
        this.keyUpEvents.filter((event) => event.key == evt.key.toUpperCase()).forEach((event) => {
            event.callback();
        });
    }

    keyDownHandler(evt: KeyboardEvent) {
        this.keyDownEvents.filter((event) => event.key == evt.key.toUpperCase()).forEach((event) => {
            event.callback();
        });
    }
        ...
        if (settingsManager.isPropRateChange) {
            ...
            if (settingsManager.isPropRateChange && !settingsManager.isAlwaysHidePropRate && timeManagerInstance.propRate0 !== timeManagerInstance.propRate) {
                if (timeManagerInstance.propRate > 1.01 || timeManagerInstance.propRate < 0.99) {
                    ...
                    if (timeManagerInstance.propRate >= 10 && timeManagerInstance.propRate < 60) uiManagerInstance.toast(`Propagation Speed: ${timeManagerInstance.propRate.toFixed(1)}x`, 'caution');

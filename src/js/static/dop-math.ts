...
export abstract class DopMath {
  ...
  public static calculateDops(azElList: AzEl<Degrees>[]): { pdop: string; hdop: string; gdop: string; vdop: string; tdop: string } {
    let dops = { pdop: '50.00', hdop: '50.00', gdop: '50.00', vdop: '50.00', tdop: '50.00' };

    let nsat = azElList.length;
    if (nsat < 4) {
      return dops;
    }
  ...
  public static updateDopsTable(dopsResults: DopList) {
    if (!dopsResults || dopsResults.length === 0) throw new Error('No DOPs results found!');
  ...
  public static getDopsList(getOffsetTimeObj: (offset: number) => Date, gpsSats: SatObject[], lat: Degrees, lon: Degrees, alt: Kilometers, el: Degrees): DopList {
    ...
    for (let t = 0; t < 1440; t++) {
      ...
      let dops = DopMath.getDops(now, gpsSats, lat, lon, alt, el);
      dopsResults.push({ time: now, dops: dops });
    }

...
gM.satrecSetup = (numOfSats, offset, now, satData) => {
  ...
  for (var i = offset; i < offset + numOfSats; i++) {
    ...
    gpuSatRecList.push(gM.transforms.satrec(satrec));
...
gM.kern.sgp4 = gM.gpu.createKernel(
  function (satrecList, tsinceList, stepSize) {
    ...
    if (satrecList[this.thread.z][16] !== 1) {
      ...
      delm = satrecList[this.thread.z][41] * (delmtemp * delmtemp * delmtemp - satrecList[this.thread.z][26]);
      temp = delomg + delm;
      mm = xmdf + temp;
      argpm = argpdf - temp;
      t3 = t2 * t;
      t4 = t3 * t;
      tempa = tempa - satrecList[this.thread.z][23] * t2 - satrecList[this.thread.z][24] * t3 - satrecList[this.thread.z][25] * t4;
      tempe = tempe + satrecList[this.thread.z][5] * satrecList[this.thread.z][22] * (Math.sin(mm) - satrecList[this.thread.z][30]);
      templ = templ + satrecList[this.thread.z][33] * t3 + t4 * (satrecList[this.thread.z][34] + t * satrecList[this.thread.z][35]);
    }
    ...
    if (pl < 0.0) {
      ...
      mrt = rl * (1.0 - 1.5 * temp2 * betal * satrecList[this.thread.z][19]) + 0.5 * temp1 * satrecList[this.thread.z][36] * cos2u;
      su = su - 0.25 * temp2 * satrecList[this.thread.z][37] * sin2u;
      xnode = nodep + 1.5 * temp2 * cosip * sin2u;
      xinc = xincp + 1.5 * temp2 * cosip * sinip * cos2u;
      var mvt = rdotl - (nm * temp1 * satrecList[this.thread.z][36] * sin2u) / xke;
      rvdot = rvdotl + (nm * temp1 * (satrecList[this.thread.z][36] * cos2u + 1.5 * satrecList[this.thread.z][19])) / xke;

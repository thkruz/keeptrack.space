...
import { Degrees, EpochUTC, J2000, Kilometers, KilometersPerSecond, RaeVec3, Satellite, TEME, Vector3D } from '@ootk/src/main';
...
export class Calculator extends KeepTrackPlugin {
        ...
        <section class="kt-section">
          ...
          <div class="input-field kt-field-row calc-control-row">
            <select id="calc-input-frame">
              ${Object.values(CoordFrame)
                .map((f) => `<option value="${f}" ${f === CoordFrame.J2000 ? 'selected' : ''}>${coordFrameLabel(f)}</option>`)
                .join('')}
            </select>
  ...
  private rebuildOutputSections_(): void {
    ...
    container.innerHTML = outputFrames
      .map((frame) => {
  ...
  private convert_(): void {
    try {
      ...
      errorManagerInstance.warn(CORE_ERROR_KEYS.has(message) ? l(`errorMsgs.${message}`) : l('errorMsgs.conversionFailed').replace('{error}', message));
    }
  }
  ...
  private renderRae_(rae: FrameOutputs['rae'], fmt: (v: number, isAngle?: boolean) => string, set: (id: string, value: string) => void): void {
    if (!rae) {
      this.lastRae_ = null;
  ...
  private renderClassical_(classical: FrameOutputs['classical'], fmt: (v: number, isAngle?: boolean) => string, set: (id: string, value: string) => void): void {
  ...
  private copyResults_(): void {
    ...
    if (o.classical && o.classical !== 'needsVelocity') {
      ...
      lines.push(
        `${coordFrameLabel(CoordFrame.CLASSICAL)}: a=${fmt(c.sma)} e=${c.ecc.toFixed(7)} i=${fmt(c.inc, true)} RAAN=${fmt(c.raan, true)} argPe=${fmt(c.argpe, true)} nu=${fmt(c.nu, true)}`
      );
    }
  ...
  private drawLine_(): void {
    ...
    ServiceLocator.getLineManager().createSensorToRae(this.sensorUsedInCalculation_, this.lastRae_);
  }

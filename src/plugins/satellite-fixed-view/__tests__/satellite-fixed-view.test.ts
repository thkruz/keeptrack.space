...
describe('SatelliteFixedView methods', () => {
  let plugin: SatelliteFixedView;
  let ssm: SelectSatManager;

  beforeEach(() => {
    setupStandardEnvironment([SelectSatManager]);
    plugin = new SatelliteFixedView();
    plugin.init();
  ...
  it('exposes config, "4" shortcut and command', () => {
    expect(plugin.getBottomIconConfig().elementName).toBeTruthy();
    expect(plugin.getKeyboardShortcuts()[0].key).toBe('4');
  ...
  it('ignores the click when already in satellite-fixed view', () => {
    ssm.selectedSat = 0;
  ...
  it('switches to FIXED_TO_SAT_LVLH when a satellite is selected', () => {
    ssm.selectedSat = 0;
    ServiceLocator.getMainCamera().cameraType = CameraType.FIXED_TO_EARTH;
    vi.spyOn(ssm, 'selectSat').mockImplementation(() => undefined);

    plugin.bottomIconCallback();
    expect(ServiceLocator.getMainCamera().cameraType).toBe(CameraType.FIXED_TO_SAT_LVLH);
  });

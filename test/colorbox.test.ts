import * as getElAll from './../src/js/lib/get-el';
import { openColorbox } from './../src/js/lib/colorbox';
...
describe('openColorbox_function', () => {
    ...
    it('test_create_colorbox', () => {
        document.getElementById('colorbox-div').remove();
        const colorboxDiv = document.getElementById('colorbox-div');
        expect(colorboxDiv).toBeNull();

        // jest replace getEl function
        jest.spyOn(getElAll, 'getEl').mockImplementationOnce(() => <HTMLElement>document.getElementById('colorbox-div'));

        openColorbox('https://www.example.com');
        const newColorboxDiv = document.getElementById('colorbox-div');
        expect(newColorboxDiv).not.toBeNull();
    });

    // Tests that loading screen is shown before opening the colorbox
    it('test_show_loading', () => {
        const loading = <HTMLElement>document.getElementById('loading-screen');
    ...
    it('test_display_colorbox', () => {
        let colorboxDiv = <HTMLElement>document.getElementById('colorbox-div');
        openColorbox('https://www.example.com');
        jest.advanceTimersByTime(2000);
        colorboxDiv = <HTMLElement>document.getElementById('colorbox-div');
    ...
    it('test_setup_image_colorbox', () => {
        ...
        const colorboxContainer = <HTMLElement>document.getElementById('colorbox-container');
        expect(colorboxContainer.style.width).toBe('45%');
        expect(colorboxContainer.style.transform).toBe('translateX(-100%)');
        const colorboxIframe = <HTMLElement>document.getElementById('colorbox-iframe');
        expect(colorboxIframe.style.display).toBe('none');
        const colorboxImg = <HTMLImageElement>document.getElementById('colorbox-img');
    ...
    it('test_setup_iframe_colorbox', () => {
        ...
        const colorboxIframe = <HTMLIFrameElement>document.getElementById('colorbox-iframe');
        expect(colorboxIframe.style.display).toBe('block');
        expect(colorboxIframe.src).toBe('https://www.example.com');
        const colorboxImg = <HTMLElement>document.getElementById('colorbox-img');
    ...
    it('test_close_colorbox', () => {
        ...
        const colorboxDom = <HTMLElement>document.getElementById('colorbox-div');

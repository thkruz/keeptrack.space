...
import { saveVariable, saveCsv, getCircularReplacer } from "@app/js/lib/saveVariable";
...
describe('code_snippet', () => {
    ...
    it('test_save_variable_default_filename', () => {
        ...
        expect(saveAsSpy).toHaveBeenCalledWith(
            new Blob([JSON.stringify(variable, getCircularReplacer())], { type: 'text/plain;charset=utf-8' }),
            expectedFilename
        );
    });
    ...
    it('test_save_csv_default_name', () => {
        ...
        expect(saveAsSpy).toHaveBeenCalledWith(
            new Blob([Papa.unparse(items)], { type: 'text/plain;charset=utf-8' }),
            expectedFilename
        );
    });
